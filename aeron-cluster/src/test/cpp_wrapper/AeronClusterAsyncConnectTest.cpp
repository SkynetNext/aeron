...
{
    const std::int64_t deadlineNs = m_aeron->context().nanoClock()() + std::chrono::hours(1).count();
    auto asyncConnect = AeronCluster::asyncConnect(m_context);
    
    EXPECT_EQ(AeronCluster::AsyncConnect::State::CREATE_EGRESS_SUBSCRIPTION, asyncConnect->state());
    EXPECT_EQ(static_cast<int>(AeronCluster::AsyncConnect::State::CREATE_EGRESS_SUBSCRIPTION), asyncConnect->step());
}
...
TEST_F(AeronClusterAsyncConnectTest, shouldCloseIngressPublicationsOnMembers)
{
    ...
    EXPECT_EQ(nullptr, asyncConnect->poll());
    EXPECT_EQ(AeronCluster::AsyncConnect::State::CREATE_INGRESS_PUBLICATIONS, asyncConnect->state());
