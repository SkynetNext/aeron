...
/**
 * Agent which coordinates consensus within a cluster in concert with the lifecycle of clustered services.
 * 
...
class ConsensusModuleAgent : 
    public ConsensusModuleControl,
    public ConsensusModuleSnapshotListener,
    public TimerService::TimerHandler
{
public:
    static constexpr std::int64_t SLOW_TICK_INTERVAL_NS = 10000000LL; // 10ms in nanoseconds
    ...
    std::shared_ptr<ClusterClientSession> getClientSession(std::int64_t clusterSessionId) override;
    ...
    void onReplaySessionMessage(std::int64_t clusterSessionId, std::int64_t timestamp);
    ...
    void onReplaySessionClose(std::int64_t clusterSessionId, CloseReason closeReason);
    ...
    ControlledPollAction onExtensionMessage(
        std::int32_t actingBlockLength,
        std::int32_t templateId,
        std::int32_t schemaId,
        std::int32_t actingVersion,
    ...
    ControlledPollAction onIngressMessage(
        std::int64_t leadershipTermId,
        std::int64_t clusterSessionId,
        AtomicBuffer& buffer,
        std::int32_t offset,
    ...
    void onSessionClose(std::int64_t leadershipTermId, std::int64_t clusterSessionId);

    void onSessionKeepAlive(std::int64_t leadershipTermId, std::int64_t clusterSessionId, Header& header);
    ...
    void onTerminationPosition(
        std::int64_t leadershipTermId,
        std::int64_t logPosition);

    void onTerminationAck(
        std::int64_t leadershipTermId,
        std::int64_t logPosition,
...
private:
    ...
    std::unordered_map<std::int64_t, std::shared_ptr<ClusterSession>> m_sessionByIdMap;
    ...
    std::vector<std::shared_ptr<PendingServiceMessageTracker>> m_pendingServiceMessageTrackers;
    ...
    void onClusterMembersQuery(std::int64_t correlationId, bool isExtendedRequest);
    ...
    std::int32_t updateFollowerPosition(
...
// Note: This is a partial implementation. Full implementation will be added incrementally.

inline ConsensusModuleAgent::ConsensusModuleAgent(ConsensusModule::Context& ctx) :
    m_ctx(ctx),
    m_aeron(ctx.aeron()),
    m_clusterClock(ctx.clusterClock()),
    m_idleStrategy(ctx.idleStrategy()),
    m_authenticator(ctx.authenticatorSupplier() ? ctx.authenticatorSupplier()->get() : nullptr),
    m_authorisationService(ctx.authorisationServiceSupplier() ? ctx.authorisationServiceSupplier()->get() : nullptr),
    m_consensusModuleExtension(ctx.consensusModuleExtension()),
    m_timerService(ctx.timerServiceSupplier()->newInstance(ctx.clusterClock()->timeUnit(), this)),
    m_markFile(ctx.clusterMarkFile()),
    m_recordingLog(ctx.recordingLog()),
    ...
    m_serviceCount(ctx.serviceCount()),
    m_memberId(ctx.clusterMemberId()),
    ...
    m_role(Role::FOLLOWER)
{
    ...
    {
        m_pendingServiceMessageTrackers[i] = std::make_shared<PendingServiceMessageTracker>(
            i, m_commitPosition, *m_logPublisher, *m_clusterClock);
    }
    
    // Determine this member
    m_thisMember = ClusterMember::determineMember(m_activeMembers, m_memberId, ctx.memberEndpoints());
    ...
    // Create adapters
    m_ingressAdapter = std::make_unique<IngressAdapter>(ctx.ingressFragmentLimit(), *this);
...
inline void ConsensusModuleAgent::onStart()
{
    ...
    {
        const std::int64_t lastTermRecordingId = m_recordingLog->findLastTermRecordingId();
    ...
    if (!m_ctx.bootstrapState())
    {
        ...
        if (m_recordingLog && m_archive)
        {
            m_recoveryPlan = std::make_shared<RecordingLog::RecoveryPlan>(
                m_recordingLog->createRecoveryPlan(m_archive, m_serviceCount, aeron::NULL_VALUE));
        }
    }
    else
    {
    ...
    ClusterMember::addConsensusPublications(
        m_activeMembers,
        m_thisMember,
        m_ctx.consensusChannel(),
        m_ctx.consensusStreamId(),
        m_ctx.enableControlOnConsensusChannel(),
        m_aeron,
    ...
    {
        const std::int64_t lastLeadershipTermId = m_recoveryPlan->lastLeadershipTermId;
        const std::int64_t commitPosition = m_commitPosition ? m_commitPosition->get() : 0;
        ...
        m_election = std::make_unique<Election>(
            ...
            m_recoveryPlan->lastTermBaseLogPosition,
            commitPosition,
            appendedPosition,
            m_activeMembers,
            m_clusterMemberByIdMap,
            m_thisMember,
            *m_consensusPublisher,
            m_ctx,
            *this);
...
inline void ConsensusModuleAgent::onClose()
{
    if (m_aeron && !m_aeron->isClosed())
    {
        ...
        {
            m_aeron->removeUnavailableCounterHandler(m_unavailableCounterHandlerRegistrationId);
        }
...
inline std::int32_t ConsensusModuleAgent::doWork()
{
    ...
    try
    {
        if (nowNs >= m_slowTickDeadlineNs)
        {
            ...
            m_slowTickDeadlineNs = slowTickWorkCount > 0 ? nowNs + 1 : nowNs + SLOW_TICK_INTERVAL_NS;
        }
...
// ConsensusModuleControl interface
inline std::int32_t ConsensusModuleAgent::memberId()
{
    return m_memberId;
}
...
inline std::shared_ptr<Aeron> ConsensusModuleAgent::aeron()
{
    return m_aeron;
}
...
inline std::shared_ptr<AuthorisationService> ConsensusModuleAgent::authorisationService()
{
    return m_authorisationService;
}

inline std::shared_ptr<ClusterClientSession> ConsensusModuleAgent::getClientSession(std::int64_t clusterSessionId)
{
    ...
    Header& header)
{
    if (m_consensusModuleExtension)
    {
        const std::int32_t remainingMessageOffset = offset + MessageHeaderEncoder::encodedLength();
        const std::int32_t remainingMessageLength = length - MessageHeaderEncoder::encodedLength();
...
inline void ConsensusModuleAgent::onReplaySessionMessage(std::int64_t clusterSessionId, std::int64_t timestamp)
{
    auto it = m_sessionByIdMap.find(clusterSessionId);
    if (it != m_sessionByIdMap.end())
    {
    ...
    {
        // Service message
        std::int32_t serviceId = PendingServiceMessageTracker::serviceIdFromLogMessage(clusterSessionId);
        if (serviceId >= 0 && serviceId < static_cast<std::int32_t>(m_pendingServiceMessageTrackers.size()))
        {
            m_pendingServiceMessageTrackers[serviceId]->sweepFollowerMessages(clusterSessionId);
        }
    }
}

inline void ConsensusModuleAgent::onReplayTimerEvent(std::int64_t correlationId)
{
    ...
    const std::string& responseChannel)
{
    auto session = std::make_shared<ClusterSession>(
        m_memberId, clusterSessionId, responseStreamId, responseChannel, responseChannel);
    session->open(logPosition);
    session->lastActivityNs(m_clusterClock->convertToNanos(timestamp), correlationId);
...
inline void ConsensusModuleAgent::onReplaySessionClose(std::int64_t clusterSessionId, CloseReason closeReason)
{
    auto it = m_sessionByIdMap.find(clusterSessionId);
    if (it != m_sessionByIdMap.end())
    {
    ...
    std::int32_t appVersion)
{
    if (m_election)
    {
        m_election->onReplayNewLeadershipTermEvent(leadershipTermId, logPosition, timestamp, termBaseLogPosition);
    }
    ...
    std::int32_t length)
{
    if (leadershipTermId == m_leadershipTermId && m_role == Role::LEADER)
    {
        ...
        if (it != m_sessionByIdMap.end() && it->second->isOpen())
        {
            ...
            std::int64_t position = m_logPublisher->appendMessage(
                leadershipTermId, clusterSessionId, timestamp, buffer, offset, length);
...
inline void ConsensusModuleAgent::onSessionConnect(
    ...
    Header& header)
{
    const std::int64_t clusterSessionId = (m_role == Role::LEADER) ? m_nextSessionId++ : aeron::NULL_VALUE;
    auto session = std::make_shared<ClusterSession>(
        m_memberId,
        clusterSessionId,
        responseStreamId,
    ...
    {
        if (client::AeronCluster::Configuration::PROTOCOL_MAJOR_VERSION != SemanticVersion::major(version))
        {
            const std::string detail = std::string(SESSION_INVALID_VERSION_MSG) + " " + 
                SemanticVersion::toString(version) + ", cluster is " + 
                SemanticVersion::toString(client::AeronCluster::Configuration::PROTOCOL_SEMANTIC_VERSION);
            session->reject(EventCode::ERROR, detail, m_ctx.errorLog());
            m_rejectedUserSessions.push_back(session);
        }
        else if (m_pendingUserSessions.size() + m_sessionByIdMap.size() >= m_ctx.maxConcurrentSessions())
        {
        ...
        else
        {
            ...
            m_authenticator->onConnectRequest(session->id(), encodedCredentials, nowNs / 1000000); // Convert to milliseconds
            m_pendingUserSessions.push_back(session);
        }
    }
}

inline void ConsensusModuleAgent::onSessionClose(std::int64_t leadershipTermId, std::int64_t clusterSessionId)
{
    if (leadershipTermId == m_leadershipTermId && m_role == Role::LEADER)
    {
        auto it = m_sessionByIdMap.find(clusterSessionId);
...
inline void ConsensusModuleAgent::onSessionKeepAlive(
    std::int64_t leadershipTermId,
    std::int64_t clusterSessionId,
    Header& header)
{
    ...
    {
        auto it = m_sessionByIdMap.find(clusterSessionId);
        if (it != m_sessionByIdMap.end() && it->second->state() == ClusterSessionState::OPEN)
        {
    ...
    const std::vector<std::uint8_t>& encodedCredentials)
{
    if (m_role == Role::LEADER)
    {
        onChallengeResponseForSession(m_pendingUserSessions, correlationId, clusterSessionId, encodedCredentials);
    }
    else
    {
        m_consensusPublisher->challengeResponse(
            m_leaderMember.publication(), correlationId, clusterSessionId, encodedCredentials);
    }
}
    ...
    std::int32_t payloadLength)
{
    ...
    if (!m_authorisationService->isAuthorised(
        ...
        AdminRequestDecoder::sbeTemplateId(), 
        requestType, 
        session->encodedPrincipal()))
    {
        const std::string msg = "Execution of the " + std::to_string(static_cast<std::int32_t>(requestType)) + " request was not authorised";
        m_egressPublisher->sendAdminResponse(
            session.get(), correlationId, requestType, AdminResponseCode::UNAUTHORISED_ACCESS, msg);
        return;
    }
    ...
    if (requestType == AdminRequestType::Value::SNAPSHOT)
    {
        ...
        {
            m_egressPublisher->sendAdminResponse(session.get(), correlationId, requestType, AdminResponseCode::OK, "");
        }
        else
        {
            const std::string msg = "Failed to switch ClusterControl to the ToggleState.SNAPSHOT state";
            m_egressPublisher->sendAdminResponse(session.get(), correlationId, requestType, AdminResponseCode::ERROR, msg);
        }
    }
    else
    {
        const std::string msg = "Unknown request type: " + std::to_string(static_cast<std::int32_t>(requestType));
        m_egressPublisher->sendAdminResponse(
            session.get(), correlationId, requestType, AdminResponseCode::ERROR, msg);
    }
}
...
inline void ConsensusModuleAgent::addSession(std::shared_ptr<ClusterSession> session)
{
    m_sessionByIdMap[session->id()] = session;
    m_sessions.push_back(session);
}

inline void ConsensusModuleAgent::closeSession(std::shared_ptr<ClusterSession> session)
{
    if (!session)
    {
        return;
    ...
    {
        m_consensusModuleExtension->onSessionClosed(sessionId, session->closeReason());
    }
}
...
inline std::int32_t ConsensusModuleAgent::slowTickWork(std::int64_t nowNs)
{
    ...
    if (!m_election)
    {
        if (Role::LEADER == role())
        {
            ...
            {
                workCount += processPendingSessions(m_pendingUserSessions, m_rejectedUserSessions, nowNs);
                workCount += processPendingSessions(m_pendingBackupSessions, m_rejectedBackupSessions, nowNs);
            ...
            else if (State::TERMINATING == state())
            {
                    ...
                    m_clusterTermination->canTerminate(m_activeMembers, nowNs))
                {
                    if (m_recordingLog)
                    {
                        m_recordingLog->commitLogPosition(m_leadershipTermId, m_terminationPosition);
                    }
                    closeAndTerminate();
                }
            }
...
inline std::int32_t ConsensusModuleAgent::consensusWork(std::int64_t timestamp, std::int64_t nowNs)
{
    std::int32_t workCount = 0;
            ...
            else
            {
                ...
                if (m_logAdapter)
                {
                    const std::int64_t limit = m_appendPosition ? 
                        m_appendPosition->get() : m_logRecordingStopPosition;
                    const std::int64_t boundPosition = std::min(m_notifiedCommitPosition, limit);
                    ...
                    if (0 == count && m_logAdapter->isImageClosed())
                    {
                        ...
                        enterElection(isEos, "log disconnected from leader: eos=" + std::to_string(isEos));
                        return 1;
                    }
...
inline void ConsensusModuleAgent::enterElection(bool isEos, const std::string& reason)
{
    if (m_election)
    {
    ...
    const std::int64_t appendedPosition = m_appendPosition ? 
        m_appendPosition->get() : std::max(
            m_recoveryPlan ? m_recoveryPlan->appendedLogPosition : 0, 
    ...
    if (m_consensusPublisher)
    {
        m_election = std::make_unique<Election>(
            false,
            isEos ? m_leaderMember.id() : aeron::NULL_VALUE,
            leadershipTermId,
            termBaseLogPosition,
            commitPosition,
            appendedPosition,
...
inline std::string ConsensusModuleAgent::refineResponseChannel(const std::string& responseChannel)
{
    ...
    else
    {
        ...
        // For now, return the original channel as template parameter copying is complex
        // This matches Java behavior where template parameters are copied
        return channelUri->toString();
    }
}

inline std::string ConsensusModuleAgent::sessionInfo(const std::string& clientInfo, Header& header)
{
    ...
    const std::string imageInfo = "sourceIdentity=" + image->sourceIdentity() + 
...
inline void ConsensusModuleAgent::onScheduleTimer(std::int64_t correlationId, std::int64_t deadline)
{
    if (m_timerService)
    {
...
inline void ConsensusModuleAgent::onUnavailableCounter(
    CountersReader& countersReader,
    std::int64_t registrationId,
    std::int32_t counterId)
{
    // Handle unavailable counter - typically used for detecting service unavailability
    // This can trigger service restart or recovery logic
}
    ...
    std::int32_t length)
{
    ...
    {
        throw ClusterException(
            "incompatible app version: required=" + std::to_string(m_ctx.appVersion()) + 
            " snapshot=" + std::to_string(appVersion),
            SOURCEINFO);
    }
    ...
    {
        throw ClusterException(
            "incompatible time unit: required=" + std::to_string(m_clusterTimeUnit) + 
    ...
    std::int32_t length)
{
    ...
    if (serviceId >= 0 && serviceId < static_cast<std::int32_t>(m_pendingServiceMessageTrackers.size()))
    {
        m_pendingServiceMessageTrackers[serviceId]->appendMessage(buffer, offset, length);
    }
}
    ...
    std::int32_t length)
{
    ...
    session->loadSnapshotState(correlationId, openedLogPosition, timeOfLastActivity, closeReason);
    addSession(session);
...
inline void ConsensusModuleAgent::onLoadTimer(
    std::int64_t correlationId,
    std::int64_t deadline,
    const util::DirectBuffer& buffer,
    std::int32_t offset,
    ...
    std::int32_t length)
{
    if (serviceId < 0 || serviceId >= static_cast<std::int32_t>(m_pendingServiceMessageTrackers.size()))
    {
        throw ClusterException(
            ...
            " invalid for serviceCount=" + std::to_string(m_pendingServiceMessageTrackers.size()),
            SOURCEINFO);
    }
...
inline void ConsensusModuleAgent::onLoadEndSnapshot(
    const util::DirectBuffer& buffer,
    std::int32_t offset,
    std::int32_t length)
{
    ...
    std::int32_t protocolVersion)
{
    ...
    if (m_election)
    {
        m_election->onRequestVote(logLeadershipTermId, logPosition, candidateTermId, candidateId, protocolVersion);
    }
    else if (candidateTermId > m_leadershipTermId)
    {
        enterElection(false, "unexpected vote request: this.leadershipTermId=" + 
            std::to_string(m_leadershipTermId) + " candidateTermId=" + std::to_string(candidateTermId));
    }
}

inline void ConsensusModuleAgent::onVote(
    std::int64_t candidateTermId,
    std::int64_t logLeadershipTermId,
    std::int64_t logPosition,
    std::int32_t candidateMemberId,
    ...
    bool isStartup)
{
    ...
    {
        enterElection(false, "unexpected new leadership term event: this.leadershipTermId=" + 
            std::to_string(m_leadershipTermId) + " newLeadershipTermId=" + std::to_string(leadershipTermId));
    }
}
    ...
    std::int16_t flags)
{
    ...
    if (m_election)
    {
        m_election->onAppendPosition(leadershipTermId, logPosition, followerMemberId, flags);
    }
...
inline void ConsensusModuleAgent::onCommitPosition(
    std::int64_t leadershipTermId,
    std::int64_t logPosition,
    std::int32_t leaderMemberId)
{
    ...
    {
        enterElection(false, "unexpected commit position from new leader - memberId=" + 
            std::to_string(m_memberId) + " this.leadershipTermId=" + std::to_string(m_leadershipTermId) +
...
inline void ConsensusModuleAgent::onStopCatchup(
    std::int64_t leadershipTermId,
    std::int32_t followerMemberId)
{
...
inline void ConsensusModuleAgent::onTerminationPosition(
    std::int64_t leadershipTermId,
    std::int64_t logPosition)
{
...
inline void ConsensusModuleAgent::onTerminationAck(
    std::int64_t leadershipTermId,
    std::int64_t logPosition,
    std::int32_t memberId)
{
...
inline void ConsensusModuleAgent::onConsensusChallengeResponse(
    ...
    const std::vector<std::uint8_t>& encodedCredentials)
{
    onChallengeResponseForSession(m_pendingBackupSessions, correlationId, clusterSessionId, encodedCredentials);
}
    ...
    const std::vector<std::uint8_t>& encodedCredentials)
{
    ...
    {
        auto session = *it;
        if (session->id() == clusterSessionId && session->state() == ClusterSessionState::CHALLENGED)
        {
            const std::int64_t nowNs = m_clusterClock->timeNanos();
            session->lastActivityNs(nowNs, correlationId);
            m_authenticator->onChallengeResponse(clusterSessionId, encodedCredentials, nowNs / 1000000); // Convert to milliseconds
            break;
        }
    }
}
...
inline void ConsensusModuleAgent::onServiceMessage(
    ...
    std::int32_t length)
{
    const std::int32_t serviceId = PendingServiceMessageTracker::serviceIdFromServiceMessage(clusterSessionId);
    if (serviceId >= 0 && serviceId < static_cast<std::int32_t>(m_pendingServiceMessageTrackers.size()))
    {
...
inline void ConsensusModuleAgent::onServiceCloseSession(std::int64_t clusterSessionId)
{
    auto it = m_sessionByIdMap.find(clusterSessionId);
    if (it != m_sessionByIdMap.end())
    {
...
inline void ConsensusModuleAgent::onServiceAck(
    std::int64_t logPosition,
    std::int64_t timestamp,
    std::int64_t ackId,
    std::int64_t relevantId,
...
inline void ConsensusModuleAgent::onClusterMembersQuery(std::int64_t correlationId, bool isExtendedRequest)
{
    ...
    else
    {
        if (m_serviceProxy)
        {
            const std::string activeMembersStr = ClusterMember::encodeAsString(m_activeMembers);
            m_serviceProxy->clusterMembersResponse(
                correlationId,
                m_leaderMember.id(),
                activeMembersStr);
        }
...
inline void ConsensusModuleAgent::replicateStandbySnapshotsForStartup()
{
    ...
    m_standbySnapshotReplicator = StandbySnapshotReplicator::newInstance(
        ...
        *archiveCtx,
        m_recordingLog,
        m_serviceCount,
        m_ctx.leaderArchiveControlChannel(),
        archiveCtx->controlRequestStreamId(),
        m_ctx.replicationChannel(),
        m_ctx.fileSyncLevel(),
        m_ctx.snapshotCounter());

    while (m_standbySnapshotReplicator && !m_standbySnapshotReplicator->isComplete())
    {
        try
        {
            const std::int32_t workCount = m_standbySnapshotReplicator->poll(m_clusterClock->timeNanos());
            m_idleStrategy->idle(workCount);
        }
...
inline std::shared_ptr<RecordingLog::RecoveryPlan> ConsensusModuleAgent::recoverFromBootstrapState()
{
    ...
    auto recoveryPlan = std::make_shared<RecordingLog::RecoveryPlan>(
        m_recordingLog->createRecoveryPlan(m_archive, m_serviceCount, m_logRecordingId));
    ...
    {
        onLoadTimer(timer.correlationId, timer.deadline, util::DirectBuffer(), 0, 0);
    }
...
{
    // Handle unavailable ingress image - typically used for detecting client disconnections
    // This can trigger cleanup of related sessions
}
...
inline std::int32_t ConsensusModuleAgent::updateLeaderPosition(std::int64_t nowNs)
{
    if (m_appendPosition)
    {
...
inline std::int32_t ConsensusModuleAgent::updateLeaderPosition(std::int64_t nowNs, std::int64_t position)
{
    ...
    const std::int64_t quorumPos = ClusterMember::quorumPosition(m_activeMembers, rankedPositions);
    const std::int64_t commitPos = std::min(quorumPos, position);
    
    const std::int64_t leaderPosition = m_commitPosition ? m_commitPosition->get() : 0;
    if (commitPos > leaderPosition ||
        (commitPos == leaderPosition && nowNs >= (m_timeOfLastLogUpdateNs + m_leaderHeartbeatIntervalNs)))
    {
        if (m_consensusPublisher)
        {
            m_consensusPublisher->commitPosition(m_leaderMember.publication(), m_leadershipTermId, commitPos, m_memberId);
        }
...
inline std::int32_t ConsensusModuleAgent::updateFollowerPosition(std::int64_t nowNs)
{
    const std::int64_t recordedPosition = m_appendPosition ? 
        m_appendPosition->get() : m_logRecordingStopPosition;
    return updateFollowerPosition(
        m_leaderMember.publication(), nowNs, m_leadershipTermId, recordedPosition, APPEND_POSITION_FLAG_NONE);
}
    ...
    std::int16_t flags)
{
    ...
    if ((position > m_lastAppendPosition ||
        nowNs >= (m_timeOfLastAppendPositionSendNs + m_leaderHeartbeatIntervalNs)))
    {
        if (m_consensusPublisher && 
            m_consensusPublisher->appendPosition(publication, leadershipTermId, position, m_memberId, flags))
        {
            if (position > m_lastAppendPosition)
            {
    ...
    std::int64_t nowNs)
{
    ...
    for (auto it = pendingSessions.rbegin(); it != pendingSessions.rend();)
    {
        ...
        {
            it = std::make_reverse_iterator(pendingSessions.erase(std::next(it).base()));
            ...
            session->state() == ClusterSessionState::CONNECTED)
        {
            if (session->isResponsePublicationConnected(m_aeron, nowNs))
            {
                ...
                if (m_authenticator)
                {
                    m_authenticator->onConnectedSession(m_sessionProxy->session(session.get()), 
        ...
        if (session->state() == ClusterSessionState::CHALLENGED)
        {
            if (session->isResponsePublicationConnected(m_aeron, nowNs))
            {
                if (m_authenticator)
                {
                    m_authenticator->onChallengedSession(m_sessionProxy->session(session.get()), 
                        m_clusterClock->timeMillis());
                }
            }
        }
            ...
            switch (session->action())
            {
                case ClusterSession::Action::CLIENT:
                {
                    if (session->appendSessionToLogAndSendOpen(
                        m_logPublisher.get(), m_egressPublisher.get(), m_leadershipTermId, m_memberId, 
                        nowNs, m_clusterClock->time()))
                    {
                ...
                case ClusterSession::Action::BACKUP:
                {
                    if (!m_authorisationService->isAuthorised(
                        MessageHeaderDecoder::sbeSchemaId(),
                        BackupQueryDecoder::sbeTemplateId(),
                        nullptr,
                        session->encodedPrincipal()))
                    {
                        session->reject(
                    ...
                    if (entry && m_consensusPublisher && m_consensusPublisher->backupResponse(
                        session.get(),
                        m_commitPosition ? m_commitPosition->id() : 0,
                        m_leaderMember.id(),
                        m_thisMember.id(),
                        *entry,
                        m_recoveryPlan.get(),
                ...
                case ClusterSession::Action::HEARTBEAT:
                {
                    if (!m_authorisationService->isAuthorised(
                        MessageHeaderDecoder::sbeSchemaId(),
                        HeartbeatRequestDecoder::sbeTemplateId(),
                        nullptr,
                        session->encodedPrincipal()))
                    {
                        session->reject(
                    ...
                    if (m_consensusPublisher && m_consensusPublisher->heartbeatResponse(session.get()))
                    {
                        it = std::make_reverse_iterator(pendingSessions.erase(std::next(it).base()));
                        session->close(m_aeron, m_ctx.countedErrorHandler(), "done");
                        workCount += 1;
                ...
                case ClusterSession::Action::STANDBY_SNAPSHOT:
                {
                    if (!m_authorisationService->isAuthorised(
                        MessageHeaderDecoder::sbeSchemaId(),
                        StandbySnapshotDecoder::sbeTemplateId(),
                        nullptr,
                        session->encodedPrincipal()))
                    {
                        session->reject(
            ...
            switch (session->state())
            {
                case ClusterSessionState::OPEN:
                {
                        ...
                        m_memberId, session.get(), m_leadershipTermId, timestamp, m_clusterTimeUnit))
                    {
                        logAppendSessionClose(
                            ...
                            timestamp,
                            m_clusterTimeUnit);
                        const std::string msg = std::to_string(static_cast<std::int32_t>(session->closeReason()));
                        if (m_egressPublisher)
                        {
                            m_egressPublisher->sendEvent(session.get(), m_leadershipTermId, m_memberId, 
                                EventCode::CLOSED, msg);
                        }
                        session->closedLogPosition(m_logPublisher->position());
                        m_uncommittedClosedSessions.push_back(session);
                        if (m_ctx.timedOutClientCounter())
                ...
                case ClusterSessionState::CLOSING:
                {
                        ...
                        m_memberId, session.get(), m_leadershipTermId, timestamp, m_clusterTimeUnit))
                    {
                        ...
                        session->closedLogPosition(m_logPublisher->position());
                        m_uncommittedClosedSessions.push_back(session);
                        if (session->closeReason() == CloseReason::TIMEOUT && m_ctx.timedOutClientCounter())
                        {
                            m_ctx.timedOutClientCounter()->incrementRelease();
                        }
                        closeSession(session);
    ...
    std::int64_t nowNs)
{
    ...
    for (auto it = redirectSessions.rbegin(); it != redirectSessions.rend();)
    {
        ...
        if (session->isResponsePublicationConnected(m_aeron, nowNs) &&
            m_egressPublisher && m_egressPublisher->sendEvent(
                session.get(), m_leadershipTermId, m_leaderMember.id(), eventCode, m_ingressEndpoints))
        {
            it = std::make_reverse_iterator(redirectSessions.erase(std::next(it).base()));
            session->close(m_aeron, m_ctx.countedErrorHandler(), std::to_string(static_cast<std::int32_t>(eventCode)));
            workCount++;
        }
    ...
    std::int64_t nowNs)
{
    ...
    for (auto it = rejectedSessions.rbegin(); it != rejectedSessions.rend();)
    {
        ...
        if (session->isResponsePublicationConnected(m_aeron, nowNs))
        {
            ...
            {
                m_egressPublisher->sendEvent(
                    session.get(), m_leadershipTermId, m_memberId, 
                    session->eventCode(), session->responseDetail());
            }
            it = std::make_reverse_iterator(rejectedSessions.erase(std::next(it).base()));
...
inline void ConsensusModuleAgent::terminateOnServiceAck(std::int64_t logPosition)
{
    if (!m_clusterTermination)
    {
        if (m_terminationLeadershipTermId == m_leadershipTermId)
        {
            if (m_consensusPublisher)
            {
                m_consensusPublisher->terminationAck(
                    m_leaderMember.publication(), m_leadershipTermId, logPosition, m_memberId);
            }
        }
        else
        {
...
inline void ConsensusModuleAgent::snapshotOnServiceAck(
    std::int64_t logPosition,
    std::int64_t timestamp,
    std::int32_t serviceId)
{
...
inline void ConsensusModuleAgent::captureServiceAck(
    std::int64_t logPosition,
    std::int64_t ackId,
    std::int64_t relevantId,
    std::int32_t serviceId)
{
    if (serviceId >= 0 && serviceId < static_cast<std::int32_t>(m_serviceAckQueues.size()))
    {
        ServiceAck::enqueue(m_serviceAckQueues[serviceId], logPosition, ackId, relevantId);
    }
}
...
inline std::int32_t ConsensusModuleAgent::checkClusterControlToggle(std::int64_t nowNs)
{
    if (State::ACTIVE == state())
    {
        if (m_controlToggle)
        {
            switch (ClusterControl::ToggleState::get(m_controlToggle))
            {
                ...
                {
                    const std::int64_t timestamp = m_clusterClock ? m_clusterClock->time() : 0;
                    if (appendAction(ClusterAction::Value::SUSPEND, timestamp, 0)) // CLUSTER_ACTION_FLAGS_DEFAULT = 0
                    {
                        state(State::SUSPENDED);
                    }
                    break;
...
inline std::int32_t ConsensusModuleAgent::pollStandbySnapshotReplication(std::int64_t nowNs)
{
    if (m_standbySnapshotReplicator && !m_standbySnapshotReplicator->isComplete())
    {
...
{
    return ClusterMember::hasActiveQuorum(m_activeMembers, m_clusterClock->timeNanos(), m_leaderHeartbeatTimeoutNs);
}

inline void ConsensusModuleAgent::sweepUncommittedEntriesTo(std::int64_t commitPosition)
{
...
inline bool ConsensusModuleAgent::appendAction(
    ClusterAction::Value action,
    std::int64_t timestamp,
    std::int32_t flags)
{
    if (m_logPublisher)
    {
        return m_logPublisher->appendClusterAction(
