...
// #include "ConsensusModule.h" // Not yet implemented, using forward
// declaration ConsensusModuleAgent.h is NOT included here to avoid circular
// dependency It will be included by ConsensusModuleAgent.h which includes this
// file
#include "LogReplay.h"
...
public:
  Election(
      ...
      ConsensusModuleContext &ctx, ConsensusModuleAgent &consensusModuleAgent);
  ...
  void onRecordingSignal(std::int64_t correlationId, std::int64_t recordingId,
                         std::int64_t position, RecordingSignal signal);
  ...
  void onTruncateLogEntry(std::int32_t memberId, ElectionState state,
                          std::int64_t logLeadershipTermId,
                          std::int64_t leadershipTermId,
                          std::int64_t candidateTermId,
                          std::int64_t commitPosition, std::int64_t logPosition,
                          std::int64_t appendPosition, std::int64_t oldPosition,
                          std::int64_t newPosition);

  std::int64_t notifiedCommitPosition() const;

  static void ensureRecordingLogCoherent(
      ConsensusModuleContext &ctx, std::int64_t recordingId,
...
private:
  ...
  void placeVote(std::int64_t candidateTermId, std::int32_t candidateId,
                 bool vote);
  std::int32_t publishNewLeadershipTermOnInterval(std::int64_t quorumPosition,
                                                  std::int64_t nowNs);
  std::int32_t publishCommitPositionOnInterval(std::int64_t quorumPosition,
                                               std::int64_t nowNs);
  void publishCanvassPosition();
  void publishNewLeadershipTerm(std::int64_t quorumPosition,
                                std::int64_t timestamp);
  void publishNewLeadershipTerm(ClusterMember &member,
                                std::int64_t logLeadershipTermId,
  ...
  void state(ElectionState newState, std::int64_t nowNs,
             const std::string &reason);
  void stopCatchup();
  void resetMembers();
  void stopReplay();
  void stopLogReplication();
  void ensureRecordingLogCoherent(std::int64_t leadershipTermId,
                                  std::int64_t termBaseLogPosition,
                                  std::int64_t logPosition, std::int64_t nowNs);
  void updateRecordingLog(std::int64_t nowNs);
  void updateRecordingLogForReplication(std::int64_t leadershipTermId,
                                        std::int64_t termBaseLogPosition,
                                        std::int64_t logPosition,
                                        std::int64_t nowNs);
  void verifyLogJoinPosition(const std::string &state,
                             std::int64_t joinPosition);
  bool hasUpdateIntervalExpired(std::int64_t nowNs, std::int64_t intervalNs);
  bool hasIntervalExpired(std::int64_t nowNs,
                          std::int64_t previousTimestampForIntervalNs,
                          std::int64_t intervalNs);
  void logStateChange(std::int32_t memberId, ElectionState oldState,
                      ElectionState newState, std::int32_t leaderId,
                      std::int64_t candidateTermId,
                      std::int64_t leadershipTermId, std::int64_t logPosition,
                      std::int64_t logLeadershipTermId,
                      std::int64_t appendPosition, std::int64_t catchupPosition,
                      const std::string &reason);
...
inline ClusterMember *Election::leader() { return m_leaderMember; }

inline std::int32_t Election::logSessionId() const { return m_logSessionId; }

inline std::int64_t Election::leadershipTermId() const {
  return m_leadershipTermId;
}

inline std::int64_t Election::logPosition() const { return m_logPosition; }

inline bool Election::isLeaderStartup() const { return m_isLeaderStartup; }

inline std::int32_t Election::thisMemberId() const { return m_thisMember.id(); }
...
inline void Election::handleError(std::int64_t nowNs,
...
inline void Election::resetMembers() { ClusterMember::reset(m_clusterMembers); }
...
inline bool Election::hasUpdateIntervalExpired(std::int64_t nowNs,
                                               std::int64_t intervalNs) {
  return hasIntervalExpired(nowNs, m_timeOfLastUpdateNs, intervalNs);
}

inline bool
Election::hasIntervalExpired(std::int64_t nowNs,
                             std::int64_t previousTimestampForIntervalNs,
                             std::int64_t intervalNs) {
...
namespace {
inline std::int32_t compareLog(std::int64_t lhsLogLeadershipTermId,
...
inline std::int32_t Election::init(std::int64_t nowNs) {
  if (m_isFirstInit) {
    ...
    if (m_logSubscription) {
      ...
      m_consensusModuleAgent.awaitLocalSocketsClosed(
          m_logSubscription->registrationId());
      m_logSubscription.reset();
    }
  }
...
inline void Election::placeVote(std::int64_t candidateTermId,
                                std::int32_t candidateId, bool vote) {
...
inline void Election::prepareForNewLeadership(std::int64_t nowNs) {
  const std::int64_t lastAppendPosition =
      m_consensusModuleAgent.prepareForNewLeadership(m_logPosition, nowNs);
  if (NULL_POSITION != lastAppendPosition) {
    m_appendPosition = lastAppendPosition;
  }
}

inline void Election::verifyLogJoinPosition(const std::string &state,
                                            std::int64_t joinPosition) {
...
inline void Election::onCanvassPosition(std::int64_t logLeadershipTermId,
                                        std::int64_t logPosition,
                                        std::int64_t leadershipTermId,
                                        std::int32_t followerMemberId,
                                        std::int32_t protocolVersion) {
  ...
  if (it != m_clusterMemberByIdMap.end() &&
    ...
    if (logLeadershipTermId < m_leadershipTermId) {
      if (service::Role::LEADER == m_consensusModuleAgent.role()) {
        publishNewLeadershipTerm(*follower, logLeadershipTermId,
...
inline void Election::onRequestVote(std::int64_t logLeadershipTermId,
                                    std::int64_t logPosition,
                                    std::int64_t candidateTermId,
...
inline void Election::onVote(std::int64_t candidateTermId,
                             std::int64_t logLeadershipTermId,
                             std::int64_t logPosition,
...
inline void Election::onAppendPosition(std::int64_t leadershipTermId,
                                       std::int64_t logPosition,
                                       std::int32_t followerMemberId,
                                       std::int16_t flags) {
  if (ElectionState::INIT == m_state) {
...
inline void Election::onCommitPosition(std::int64_t leadershipTermId,
                                       std::int64_t logPosition,
                                       std::int32_t leaderMemberId) {
...
inline std::int32_t Election::leaderReplay(std::int64_t nowNs) {
  ...
  if (!m_logReplay) {
    if (m_logPosition < m_appendPosition) {
      m_logReplay =
          m_consensusModuleAgent.newLogReplay(m_logPosition, m_appendPosition);
    } else {
...
inline std::int32_t Election::leaderReady(std::int64_t nowNs) {
  std::int32_t workCount =
      m_consensusModuleAgent.updateLeaderPosition(nowNs, m_appendPosition);
  workCount += publishNewLeadershipTermOnInterval(
      m_consensusModuleAgent.quorumPosition(), nowNs);
...
Election::publishNewLeadershipTermOnInterval(std::int64_t quorumPosition,
  ...
  if (hasUpdateIntervalExpired(nowNs, m_ctx.leaderHeartbeatIntervalNs())) {
    m_timeOfLastUpdateNs = nowNs;
    publishNewLeadershipTerm(quorumPosition,
                             m_ctx.clusterClock()->convertToTimeUnit(nowNs));
    workCount++;
  }
...
inline void Election::publishNewLeadershipTerm(std::int64_t quorumPosition,
                                               std::int64_t timestamp) {
  for (auto &member : m_clusterMembers) {
...
inline void Election::publishNewLeadershipTerm(ClusterMember &member,
                                               std::int64_t logLeadershipTermId,
                                               std::int64_t quorumPosition,
                                               std::int64_t timestamp) {
  if (member.id() != m_thisMember.id() &&
      aeron::NULL_SESSION_ID != m_logSessionId) {
...
inline std::int32_t
Election::publishFollowerReplicationPosition(std::int64_t nowNs) {
...
inline std::int32_t
Election::publishFollowerAppendPosition(std::int64_t nowNs) {
...
inline void Election::addLiveLogDestination() {
  ...
  if (m_ctx.isLogMdc()) {
    destination = ChannelUri::createDestinationUri(m_ctx.logChannel(),
                                                   m_thisMember.logEndpoint());
  } else {
