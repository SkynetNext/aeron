...
// Forward declaration for test fixture (only defined in test files, in global
// namespace)
 ...
 */
class EgressAdapter {
  // Allow test classes to access private onFragment method for 1:1 test
  // translation
  friend class ::EgressAdapterTest;

public:
  /**
   * Construct an adapter for cluster egress which consumes from the
   * subscription and dispatches to the EgressListener.
   *
   ...
   */
  EgressAdapter(std::shared_ptr<EgressListener> listener,
                std::int64_t clusterSessionId,
                std::shared_ptr<Subscription> subscription, int fragmentLimit)
      : EgressAdapter(listener, nullptr, clusterSessionId, subscription,
                      fragmentLimit) {}

  /**
   * Construct an adapter for cluster egress which consumes from the
   * subscription and dispatches to the EgressListener or extension messages to
   * EgressListenerExtension.
   *
   * @param listener          to dispatch events to.
  ...
  EgressAdapter(std::shared_ptr<EgressListener> listener,
                ...
                std::int64_t clusterSessionId,
                std::shared_ptr<Subscription> subscription, int fragmentLimit)
      : m_clusterSessionId(clusterSessionId), m_fragmentLimit(fragmentLimit),
        m_listener(listener), m_listenerExtension(listenerExtension),
        m_subscription(subscription),
        m_fragmentAssembler([this](AtomicBuffer &buffer, util::index_t offset,
...
public:
  // Made public for 1:1 test translation (friend declaration not working
  // reliably)
  void onFragment(AtomicBuffer &buffer, std::int32_t offset,
                  std::int32_t length, Header &header) {
    m_messageHeaderDecoder.wrap(reinterpret_cast<char *>(buffer.buffer()),
                                offset, MessageHeader::sbeSchemaVersion(),
                                buffer.capacity());

    const std::int32_t templateId = m_messageHeaderDecoder.templateId();
    const std::int32_t schemaId = m_messageHeaderDecoder.schemaId();
    // Note: MessageHeader::sbeSchemaId() incorrectly returns 112 due to code
    // generation issue The correct schema ID for cluster codecs is 111 (from
    // aeron-cluster-codecs.xml) Use SessionMessageHeader::sbeSchemaId() which
    // correctly returns 111
    if (schemaId != SessionMessageHeader::sbeSchemaId()) {
      if (m_listenerExtension) {
        m_listenerExtension->onExtensionMessage(
            m_messageHeaderDecoder.blockLength(), templateId, schemaId,
            m_messageHeaderDecoder.version(), buffer,
      ...
      throw ClusterException(
          "expected schemaId=" +
              std::to_string(SessionMessageHeader::sbeSchemaId()) +
    ...
    case SessionMessageHeader::SBE_TEMPLATE_ID: {
      m_sessionMessageHeaderDecoder.wrapForDecode(
          ...
          m_messageHeaderDecoder.version(), buffer.capacity());
      ...
      if (sessionId == m_clusterSessionId) {
        m_listener->onMessage(
            sessionId, m_sessionMessageHeaderDecoder.timestamp(), buffer,
            offset + AeronCluster::SESSION_HEADER_LENGTH,
            length - AeronCluster::SESSION_HEADER_LENGTH, header);
      }
      break;
    }
    ...
    case SessionEvent::SBE_TEMPLATE_ID: {
      ...
      if (sessionId == m_clusterSessionId) {
        m_listener->onSessionEvent(
            m_sessionEventDecoder.correlationId(), sessionId,
            m_sessionEventDecoder.leadershipTermId(),
            m_sessionEventDecoder.leaderMemberId(),
            m_sessionEventDecoder.code(), m_sessionEventDecoder.detail());
      }
      break;
    }
    ...
    case NewLeaderEvent::SBE_TEMPLATE_ID: {
      ...
      if (sessionId == m_clusterSessionId) {
        m_listener->onNewLeader(sessionId,
    ...
    case AdminResponse::SBE_TEMPLATE_ID: {
      ...
      if (sessionId == m_clusterSessionId) {
        const std::int64_t correlationId =
            m_adminResponseDecoder.correlationId();
        const AdminRequestType::Value requestType =
            m_adminResponseDecoder.requestType();
        const AdminResponseCode::Value responseCode =
            m_adminResponseDecoder.responseCode();
        // Use messageLength() to correctly construct string, as message()
        // returns const char* which may include extra bytes if payload follows
        // immediately
        const std::uint32_t messageLength =
            m_adminResponseDecoder.messageLength();
        const std::string message(m_adminResponseDecoder.message(),
                                  messageLength);
        const std::int32_t payloadOffset =
            m_adminResponseDecoder.offset() + AdminResponse::SBE_BLOCK_LENGTH +
            AdminResponse::messageHeaderLength() +
            static_cast<std::int32_t>(messageLength) +
            AdminResponse::payloadHeaderLength();
        const std::int32_t payloadLength =
            m_adminResponseDecoder.payloadLength();
        m_listener->onAdminResponse(sessionId, correlationId, requestType,
                                    responseCode, message, buffer,
