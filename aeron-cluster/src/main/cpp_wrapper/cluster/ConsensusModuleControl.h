...
#include "ConsensusModule.h" // Must include before namespace for ConsensusModule::Context
...
/**
 * Control interface for performing operations on the consensus module from a
...
public:
   ...
   *
   * @return the unit of time applied when timestamping and invoking time()
   * operations.
   * @see time()
   */
  ...
  /**
   * IdleStrategy which should be used by the extension when it experiences
   * back-pressure or is undertaking any long-running actions.
   *
   * @return the IdleStrategy which should be used by the extension when it
   * experiences back-pressure or is undertaking any long-running actions.
   */
   ...
   */
  virtual std::shared_ptr<archive::client::AeronArchive> archive() = 0;
   ...
   */
  virtual std::shared_ptr<security::AuthorisationService>
  authorisationService() = 0;

  /**
   * Lookup a ClusterClientSession for a given id.
   *
   * @param clusterSessionId for the session to lookup.
   * @return a ClusterClientSession for a given id, otherwise nullptr if not
   * found.
   */
  virtual std::shared_ptr<ClusterClientSession>
  ...
  /**
   * Numeric id for the cluster (used when running multiple clusters on the same
   * media driver).
   *
