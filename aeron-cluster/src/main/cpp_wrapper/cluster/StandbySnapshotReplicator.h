...
public:
  ...
  struct SnapshotReplicationEntry {
    ...
    SnapshotReplicationEntry(const std::string &endpoint,
                             std::int64_t logPosition,
                             const std::vector<RecordingLog::Entry> &entries)
        : endpoint(endpoint), logPosition(logPosition),
          recordingLogEntries(entries) {}
  };

  static std::unique_ptr<StandbySnapshotReplicator>
  newInstance(std::int32_t memberId, const archive::client::Context &archiveCtx,
              RecordingLog &recordingLog, std::int32_t serviceCount,
              const std::string &archiveControlChannel,
              std::int32_t archiveControlStreamId,
              const std::string &replicationChannel, std::int32_t fileSyncLevel,
              std::shared_ptr<concurrent::AtomicCounter> snapshotCounter);
  ...
  void onSignal(std::int64_t controlSessionId, std::int64_t correlationId,
                std::int64_t recordingId, std::int64_t subscriptionId,
...
private:
  StandbySnapshotReplicator(
      std::int32_t memberId, std::shared_ptr<AeronArchive> archive,
      RecordingLog &recordingLog, std::int32_t serviceCount,
      const std::string &archiveControlChannel,
      std::int32_t archiveControlStreamId,
      const std::string &replicationChannel, std::int32_t fileSyncLevel,
  ...
  static int compareTo(const SnapshotReplicationEntry &a,
                       const SnapshotReplicationEntry &b);

  void logReplicationEnded(const std::string &controlUri,
                           std::int64_t srcRecordingId,
                           std::int64_t dstRecordingId, std::int64_t position,
                           bool hasSynced);

  std::unordered_map<std::string, std::vector<RecordingLog::Entry>>
  filterByExistingRecordingLogEntries(
      const std::unordered_map<std::string, std::vector<RecordingLog::Entry>>
          &standbySnapshotsByEndpoint);
  ...
  struct EventListenerWrapper
      : public MultipleRecordingReplication::EventListener {
    StandbySnapshotReplicator *m_replicator;

    explicit EventListenerWrapper(StandbySnapshotReplicator *r)
        : m_replicator(r) {}

    void onReplicationEnded(const std::string &controlUri,
                            std::int64_t srcRecordingId,
                            std::int64_t dstRecordingId, std::int64_t position,
                            bool hasSynced) override {
      m_replicator->logReplicationEnded(controlUri, srcRecordingId,
                                        dstRecordingId, position, hasSynced);
    }
  };
...
// Implementation
inline std::unique_ptr<StandbySnapshotReplicator>
StandbySnapshotReplicator::newInstance(
    std::int32_t memberId, const archive::client::Context &archiveCtx,
    RecordingLog &recordingLog, std::int32_t serviceCount,
    const std::string &archiveControlChannel,
    std::int32_t archiveControlStreamId, const std::string &replicationChannel,
  ...
  archiveCtxClone.recordingSignalConsumer([&replicatorShared](
    ...
    if (replicatorShared) {
      // correlationId is not available in RecordingSignal, use 0 as default
      replicatorShared->onSignal(signal.m_controlSessionId,
                                 0, // correlationId - not available in signal
                                 signal.m_recordingId, signal.m_subscriptionId,
                                 signal.m_position, signal);
    }
  });
  ...
  auto replicator =
      std::unique_ptr<StandbySnapshotReplicator>(new StandbySnapshotReplicator(
          memberId, archive, recordingLog, serviceCount, archiveControlChannel,
          archiveControlStreamId, replicationChannel, fileSyncLevel,
          snapshotCounter));

  // Store shared pointer for lambda capture
  replicatorShared = std::shared_ptr<StandbySnapshotReplicator>(
      replicator.get(), [](StandbySnapshotReplicator
                               *) { /* no-op deleter, owned by unique_ptr */ });
...
inline StandbySnapshotReplicator::StandbySnapshotReplicator(
    ...
    std::int32_t fileSyncLevel,
    std::shared_ptr<concurrent::AtomicCounter> snapshotCounter)
    : m_memberId(memberId), m_archive(archive), m_recordingLog(recordingLog),
      m_serviceCount(serviceCount),
      m_archiveControlChannel(archiveControlChannel),
      m_archiveControlStreamId(archiveControlStreamId),
      m_replicationChannel(replicationChannel), m_fileSyncLevel(fileSyncLevel),
      m_snapshotCounter(snapshotCounter), m_eventListenerWrapper(this) {}

inline StandbySnapshotReplicator::~StandbySnapshotReplicator() { close(); }
...
inline std::int32_t StandbySnapshotReplicator::poll(std::int64_t nowNs) {
  ...
  if (!m_recordingReplication) {
    ...
    if (m_snapshotsToReplicate.empty()) {
      std::string errorMsg =
          "failed to replicate any standby snapshots, errors: ";
      for (const auto &pair : m_errorsByEndpoint) {
        errorMsg += pair.first + "=" + pair.second + ", ";
      }
      throw client::ClusterException(errorMsg, SOURCEINFO);
    }
    ...
    const std::int64_t progressTimeoutNs =
        m_archive->context().messageTimeoutNs() * 2;
    const std::int64_t progressIntervalNs = progressTimeoutNs / 10;

    m_recordingReplication = MultipleRecordingReplication::newInstance(
        m_archive, m_archiveControlStreamId, srcChannel, m_replicationChannel,
        progressTimeoutNs, progressIntervalNs);

    m_recordingReplication->setEventListener(&m_eventListenerWrapper);

    for (const auto &entry :
         m_currentSnapshotToReplicate->recordingLogEntries) {
      m_recordingReplication->addRecording(
          entry.recordingId,
          aeron::NULL_VALUE,               // NULL_RECORDING_ID
          archive::client::NULL_POSITION); // NULL_POSITION
    }
  ...
  if (m_recordingReplication && m_recordingReplication->isComplete()) {
    for (const auto &entry :
         m_currentSnapshotToReplicate->recordingLogEntries) {
      const std::int64_t dstRecordingId =
          m_recordingReplication->completedDstRecordingId(entry.recordingId);
      m_recordingLog.appendSnapshot(
          dstRecordingId, entry.leadershipTermId, entry.termBaseLogPosition,
          entry.logPosition, entry.timestamp, entry.serviceId);
    }
    m_recordingLog.force(m_fileSyncLevel);

    m_snapshotCounter->increment();
...
inline void StandbySnapshotReplicator::onSignal(
    std::int64_t /* controlSessionId */, std::int64_t correlationId,
    std::int64_t recordingId, std::int64_t /* subscriptionId */,
    std::int64_t position, RecordingSignal signal) {
  if (m_recordingReplication) {
    m_recordingReplication->onSignal(correlationId, recordingId, position,
                                     signal);
  }
}
...
inline std::vector<StandbySnapshotReplicator::SnapshotReplicationEntry>
StandbySnapshotReplicator::computeSnapshotsToReplicate() {
  const auto latestSnapshots =
      m_recordingLog.latestStandbySnapshots(m_serviceCount);
  std::unordered_map<std::string, std::vector<RecordingLog::Entry>>
      snapshotsMap(latestSnapshots.begin(), latestSnapshots.end());
  const auto snapshotsByEndpoint =
      filterByExistingRecordingLogEntries(snapshotsMap);
  ...
  for (const auto &pair : snapshotsByEndpoint) {
    ...
    orderedSnapshotToReplicate.emplace_back(pair.first, logPosition,
                                            pair.second);
  }

  std::sort(orderedSnapshotToReplicate.begin(),
            orderedSnapshotToReplicate.end(), compareTo);

  return orderedSnapshotToReplicate;
}

inline int
StandbySnapshotReplicator::compareTo(const SnapshotReplicationEntry &a,
                                     const SnapshotReplicationEntry &b) {
  const int descendingOrderCompare =
      -static_cast<int>(a.logPosition - b.logPosition);
...
inline void StandbySnapshotReplicator::logReplicationEnded(
    const std::string & /*controlUri*/, std::int64_t /*srcRecordingId*/,
    std::int64_t /*dstRecordingId*/, std::int64_t /*position*/,
    bool /*hasSynced*/) {
  // This method corresponds to ConsensusModuleAgent::logReplicationEnded in
  // Java, which is currently an empty method (placeholder for future logging
  // functionality). See ConsensusModuleAgent.java line 2412-2421
}

inline std::unordered_map<std::string, std::vector<RecordingLog::Entry>>
StandbySnapshotReplicator::filterByExistingRecordingLogEntries(
    const std::unordered_map<std::string, std::vector<RecordingLog::Entry>>
        &standbySnapshotsByEndpoint) {
  std::unordered_map<std::string, std::vector<RecordingLog::Entry>>
      filteredSnapshotsByEndpoint;
  ...
  for (const auto &pair : standbySnapshotsByEndpoint) {
    ...
    for (auto it = pair.second.rbegin(); it != pair.second.rend(); ++it) {
      ...
      auto *snapshotEntry =
          m_recordingLog.getLatestSnapshot(standbySnapshotEntry.serviceId);
      if (snapshotEntry &&
          standbySnapshotEntry.logPosition <= snapshotEntry->logPosition) {
        // Skip this entry
      } else {
