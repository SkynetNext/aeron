...
inline Role getRole(std::int64_t code) {
  if (code < 0 || code > 2) {
    throw IllegalStateException(
        "Invalid role counter code: " + std::to_string(code), SOURCEINFO);
  }
 ...
 */
inline Role
getRole(std::shared_ptr<::aeron::concurrent::AtomicCounter> counter) {
  if (!counter) {
...
public:
  ...
  /**
   * The unique id for the hosting member of the cluster. Useful only for
   * debugging purposes.
   *
   ...
   */
  virtual std::shared_ptr<::aeron::Aeron> aeron() = 0;

  /**
   * Get the ClusteredServiceContainer::Context under which the container is
   * running.
   *
   * @return the ClusteredServiceContainer::Context under which the container is
   * running.
   */
  ...
  getClientSession(std::int64_t clusterSessionId) = 0;
  ...
  /**
   * For each iterator over ClientSessions using the most efficient method
   * possible.
   *
   * @param action to be taken for each ClientSession in turn.
   */
  virtual void forEachClientSession(
      std::function<void(std::shared_ptr<ClientSession>)> action) = 0;

  /**
   * Request the close of a ClientSession by sending the request to the
   * consensus module.
   *
   * @param clusterSessionId to be closed.
   * @return true if the event to close a session was sent or false if back
  ...
  /**
   * Schedule a timer for a given deadline and provide a correlation id to
   * identify the timer when it expires or for cancellation.
   *
   * @param correlationId to identify the timer when it expires. Long::MAX_VALUE
   * not supported.
   * @param deadline      time after which the timer will fire. Long::MAX_VALUE
   * not supported.
   * @return true if the request to schedule a timer has been sent or false in
   * case of ADMIN_ACTION or BACK_PRESSURED.
   * @throws ClusterException if request fails with an error.
   */
  virtual bool scheduleTimer(std::int64_t correlationId,
   ...
   * Long::MAX_VALUE not supported.
   * @return true if the request to cancel a timer has been sent or false in
   * case of ADMIN_ACTION or BACK_PRESSURED.
   * @throws ClusterException if request fails with an error.
   */
   ...
   */
  virtual std::int64_t offer(const util::DirectBuffer &buffer,
                             std::int32_t offset, std::int32_t length) = 0;

  /**
   * Try to claim a range in the publication log into which a message can be
   * written with zero copy semantics.
   *
   * @param length      of the range to claim, in bytes.
   * @param bufferClaim to be populated if the claim succeeds.
   * @return positive value if successful.
   * @throws IllegalArgumentException if the length is greater than
   * Publication::maxPayloadLength().
   */
  virtual std::int64_t tryClaim(std::int32_t length,
                                BufferClaim &bufferClaim) = 0;

  /**
   * IdleStrategy which should be used by the service when it experiences
   * back-pressure on egress, closing sessions, making timer requests, or any
   * long-running actions.
   *
   * @return the IdleStrategy which should be used by the service when it
   * experiences back-pressure.
   */
  virtual std::shared_ptr<::aeron::concurrent::IdleStrategy> idleStrategy() = 0;
};
