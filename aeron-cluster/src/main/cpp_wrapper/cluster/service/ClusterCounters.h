...
public:
   ...
   */
  static std::shared_ptr<Counter> allocate(std::shared_ptr<Aeron> aeron,
    ...
    std::int64_t registrationId =
        aeron->addCounter(typeId, tempBuffer.buffer(), keyLength, label);
    return aeron->findCounter(registrationId);
  }

  /**
   * Allocate a counter to represent component state within a cluster and append
   * a version info to its label.
   *
   * @param aeron          to allocate the counter.
   * @param tempBuffer     temporary storage to create label and metadata.
   * @param name           of the counter for the label.
   ...
   * @return the new Counter.
   */
  static std::shared_ptr<Counter>
  allocateVersioned(std::shared_ptr<Aeron> aeron, AtomicBuffer &tempBuffer,
                    const std::string &name, std::int32_t typeId,
                    std::int32_t clusterId, const std::string &version,
    ...
    std::string versionInfo =
        " version=" + version + " commit=" + commitHashCode;
    index += tempBuffer.putStringWithoutLength(index, versionInfo);

    std::string label = tempBuffer.getStringWithoutLength(
        keyLength, static_cast<std::size_t>(index - keyLength));
    std::int64_t registrationId =
        aeron->addCounter(typeId, tempBuffer.buffer(), keyLength, label);
    return aeron->findCounter(registrationId);
  }
   ...
   */
  static std::int32_t find(CountersReader &counters, std::int32_t typeId,
                           std::int32_t clusterId) {
    ...
    for (std::int32_t i = 0, size = counters.maxCounterId(); i < size; i++) {
      ...
      if (counterState == CountersReader::RECORD_ALLOCATED) {
        if (counters.getCounterTypeId(i) == typeId &&
            buffer.getInt32(CountersReader::metadataOffset(i) +
                            CountersReader::KEY_OFFSET) == clusterId) {
          return i;
        }
   ...
   */
  static std::shared_ptr<Counter>
  allocateServiceCounter(std::shared_ptr<Aeron> aeron, AtomicBuffer &tempBuffer,
                         const std::string &name, std::int32_t typeId,
                         std::int32_t clusterId, std::int32_t serviceId) {
    std::int32_t index = 0;
    tempBuffer.putInt32(index, clusterId);
    index += sizeof(std::int32_t);
  ...
  static std::shared_ptr<Counter> allocateServiceErrorCounter(
      std::shared_ptr<Aeron> aeron, AtomicBuffer &tempBuffer,
      std::int32_t clusterId, std::int32_t serviceId,
      const std::string &version, const std::string &gitSha) {
    std::int32_t index = 0;
    tempBuffer.putInt32(index, clusterId);
    index += sizeof(std::int32_t);
    tempBuffer.putInt32(index, serviceId);
    ...
    index +=
        tempBuffer.putStringWithoutLength(index, "Cluster Container Errors");
    index += tempBuffer.putStringWithoutLength(index, CLUSTER_ID_LABEL_SUFFIX);
    std::string clusterIdStr = std::to_string(clusterId);
    index += tempBuffer.putStringWithoutLength(index, clusterIdStr);
    index += tempBuffer.putStringWithoutLength(index, SERVICE_ID_SUFFIX);
    ...
    std::int64_t registrationId = aeron->addCounter(
        ...
        tempBuffer.buffer(), keyLength, label);
    return aeron->findCounter(registrationId);
  }
};
