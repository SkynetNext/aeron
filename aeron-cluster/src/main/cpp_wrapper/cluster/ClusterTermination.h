...
public:
  ...
  bool canTerminate(const std::vector<ClusterMember> &members,
                    std::int64_t nowNs);

  void onServicesTerminated();

  void terminationPosition(const exception_handler_t &errorHandler,
...
// Implementation
inline ClusterTermination::ClusterTermination(std::int64_t deadlineNs,
                                              std::int32_t serviceCount)
    : m_deadlineNs(deadlineNs), m_haveServicesTerminated(serviceCount <= 0) {}
...
inline bool
ClusterTermination::canTerminate(const std::vector<ClusterMember> &members,
                                 std::int64_t nowNs) {
...
inline void ClusterTermination::terminationPosition(
    ...
    ConsensusPublisher &consensusPublisher,
    const std::vector<ClusterMember> &members, const ClusterMember &thisMember,
    std::int64_t leadershipTermId, std::int64_t position) {
  for (auto &member : const_cast<std::vector<ClusterMember> &>(members)) {
    ...
    if (member.id() != thisMember.id()) {
      ...
      if (publication) {
        if (!consensusPublisher.terminationPosition(
                publication, leadershipTermId, position)) {
          ClusterEvent event("failed to send termination position to member=" +
