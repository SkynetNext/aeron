...
#include "Election.h"
#include "ChannelUriStringBuilder.h" // For ChannelUriStringBuilder
#include "ConsensusModuleAgent.h"    // For ConsensusModule::Context definition
#include "ConsensusPublisher.h"      // For ConsensusPublisher
#include "client/archive/AeronArchive.h" // For NULL_POSITION
#include <thread>

namespace aeron {
namespace cluster {

// No type alias needed - ConsensusModule::Context is already defined

// Implementation
Election::Election(
    bool isNodeStartup, std::int32_t gracefulClosedLeaderId,
    std::int64_t leadershipTermId, std::int64_t termBaseLogPosition,
    std::int64_t logPosition, std::int64_t appendPosition,
    ...
    : m_clusterMembers(clusterMembers), m_thisMember(thisMember),
      m_clusterMemberByIdMap(clusterMemberByIdMap),
      m_consensusPublisher(consensusPublisher), m_ctx(ctx),
      ...
      m_isNodeStartup(isNodeStartup), m_logPosition(logPosition),
      m_appendPosition(appendPosition), m_logLeadershipTermId(leadershipTermId),
      m_leadershipTermId(leadershipTermId), m_candidateTermId(leadershipTermId),
  ...
  m_initialTimeOfLastUpdateNs =
      nowNs - (24LL * 60 * 60 * 1000 * 1000 * 1000); // 1 day in nanoseconds
  m_timeOfLastUpdateNs = m_initialTimeOfLastUpdateNs;
  m_timeOfLastCommitPositionUpdateNs = m_initialTimeOfLastUpdateNs;

  ctx.electionStateCounter()->setRelease(
      static_cast<std::int64_t>(ElectionState::INIT));
  ctx.electionCounter()->incrementRelease();

  if (clusterMembers.size() == 1 && thisMember.id() == clusterMembers[0].id()) {
    m_candidateTermId =
        std::max(leadershipTermId + 1,
                 ctx.nodeStateFile()->candidateTerm().candidateTermId() + 1);
    m_leadershipTermId = m_candidateTermId;
    m_leaderMember = &thisMember;
    ctx.nodeStateFile()->updateCandidateTermId(m_candidateTermId, logPosition,
                                               ctx.epochClock()->time());
    state(ElectionState::LEADER_LOG_REPLICATION, nowNs, "");
  }
}

ClusterMember *Election::leader() { return m_leaderMember; }

std::int32_t Election::logSessionId() const { return m_logSessionId; }

std::int64_t Election::leadershipTermId() const { return m_leadershipTermId; }

std::int64_t Election::logPosition() const { return m_logPosition; }

bool Election::isLeaderStartup() const { return m_isLeaderStartup; }

std::int32_t Election::thisMemberId() const { return m_thisMember.id(); }

std::int32_t Election::doWork(std::int64_t nowNs) {
...
void Election::handleError(std::int64_t nowNs, const std::exception &ex) {
...
std::int64_t Election::notifiedCommitPosition() const {
...
void Election::resetMembers() { ClusterMember::reset(m_clusterMembers); }

void Election::stopReplay() {
  if (m_logReplay) {
    m_logReplay->close();
    m_logReplay.reset();
  }
...
void Election::stopLogReplication() {
  if (m_logReplication) {
...
namespace {
std::int32_t compareLog(std::int64_t lhsLogLeadershipTermId,
...
std::int32_t Election::init(std::int64_t nowNs) {
  if (m_isFirstInit) {
    m_isFirstInit = false;
    if (!m_isNodeStartup) {
    ...
    if (m_logSubscription) {
      const std::int64_t registrationId = m_logSubscription->registrationId();
      m_logSubscription.reset();
      m_consensusModuleAgent.awaitLocalSocketsClosed(registrationId);
    }
  }

  m_notifiedCommitPosition = 0;
  m_candidateTermId =
      std::max(m_ctx.nodeStateFile()->candidateTerm().candidateTermId(),
               m_leadershipTermId);

  if (m_clusterMembers.size() == 1 &&
      m_thisMember.id() == m_clusterMembers[0].id()) {
    state(ElectionState::LEADER_LOG_REPLICATION, nowNs, "");
  } else {
...
std::int32_t Election::canvass(std::int64_t nowNs) {
  std::int32_t workCount = 0;
  const std::int64_t deadlineNs =
      m_isExtendedCanvass
          ? m_timeOfLastStateChangeNs + m_ctx.startupCanvassTimeoutNs()
          : m_consensusModuleAgent.timeOfLastLeaderUpdateNs() +
                m_ctx.leaderHeartbeatTimeoutNs();
  ...
  if (m_ctx.appointedLeaderId() != aeron::NULL_VALUE &&
      m_ctx.appointedLeaderId() != m_thisMember.id()) {
    return workCount;
  }
  ...
  const std::int32_t quorumThreshold =
      (static_cast<std::int32_t>(m_clusterMembers.size()) / 2) + 1;
  ...
  if (isUnanimous || (nowNs >= deadlineNs && isQuorum)) {
    ...
    const std::int64_t delayNs = static_cast<std::int64_t>(
        delayFactor * (m_ctx.electionTimeoutNs() >> 1));
...
std::int32_t Election::nominate(std::int64_t nowNs) {
  if (nowNs >= m_nominationDeadlineNs) {
    ...
    return 1;
  } else if (hasUpdateIntervalExpired(nowNs,
                                      m_ctx.electionStatusIntervalNs())) {
    m_timeOfLastUpdateNs = nowNs;
    publishCanvassPosition();
    return 1;
  }
...
std::int32_t Election::candidateBallot(std::int64_t nowNs) {
  std::int32_t workCount = 0;
    ...
    for (auto &member : m_clusterMembers) {
      if (!member.isBallotSent()) {
        ...
        const bool sent = m_consensusPublisher.requestVote(
            member.publication(), m_logLeadershipTermId, m_appendPosition,
            m_candidateTermId, m_thisMember.id());
...
std::int32_t Election::followerBallot(std::int64_t nowNs) {
  std::int32_t workCount = 0;
...
void Election::publishCanvassPosition() {
  for (auto &member : m_clusterMembers) {
    if (member.id() != m_thisMember.id()) {
      ...
      m_consensusPublisher.canvassPosition(
          member.publication(), m_logLeadershipTermId, m_appendPosition,
          m_leadershipTermId, m_thisMember.id());
    }
  }
}

void Election::placeVote(std::int64_t candidateTermId, std::int32_t candidateId,
                         bool vote) {
  auto it = m_clusterMemberByIdMap.find(candidateId);
  if (it != m_clusterMemberByIdMap.end()) {
    ClusterMember *candidate = it->second;
    m_consensusPublisher.placeVote(candidate->publication(), candidateTermId,
                                   m_logLeadershipTermId, m_appendPosition,
                                   candidateId, m_thisMember.id(), vote);
  }
}

void Election::prepareForNewLeadership(std::int64_t nowNs) {
  const std::int64_t lastAppendPosition =
      m_consensusModuleAgent.prepareForNewLeadership(m_logPosition);
  if (NULL_POSITION != lastAppendPosition) {
    m_appendPosition = lastAppendPosition;
  }
}

void Election::verifyLogJoinPosition(const std::string &state,
                                     std::int64_t joinPosition) {
  if (joinPosition != m_logPosition) {
    const std::string inequality =
        joinPosition < m_logPosition ? " less " : " greater ";
    throw ClusterEvent(
        state + " - joinPosition=" + std::to_string(joinPosition) + inequality +
            "than logPosition=" + std::to_string(m_logPosition),
        SOURCEINFO);
  }
}

void Election::logStateChange(
    std::int32_t memberId, ElectionState oldState, ElectionState newState,
    std::int32_t leaderId, std::int64_t candidateTermId,
    std::int64_t leadershipTermId, std::int64_t logPosition,
    std::int64_t logLeadershipTermId, std::int64_t appendPosition,
    std::int64_t catchupPosition, const std::string &reason) {
  // Log state change - in Java this is commented out, so we'll leave it empty
  // for now Can be implemented with a logger if needed
}

void Election::ensureRecordingLogCoherent(std::int64_t leadershipTermId,
                                          std::int64_t termBaseLogPosition,
                                          std::int64_t logPosition,
                                          std::int64_t nowNs) {
  ensureRecordingLogCoherent(m_ctx, m_consensusModuleAgent.logRecordingId(),
                             m_initialLogLeadershipTermId,
                             m_initialTermBaseLogPosition, leadershipTermId,
                             termBaseLogPosition, logPosition, nowNs);
}

void Election::ensureRecordingLogCoherent(
    ConsensusModule::Context &ctx, std::int64_t recordingId,
    std::int64_t initialLogLeadershipTermId,
    std::int64_t initialTermBaseLogPosition, std::int64_t leadershipTermId,
  ...
  const std::int64_t timestamp = ctx.clusterClock()->time();
  auto recordingLog = ctx.recordingLog();

  recordingLog->ensureCoherent(
      recordingId, initialLogLeadershipTermId, initialTermBaseLogPosition,
      leadershipTermId,
      (aeron::NULL_VALUE != termBaseLogPosition) ? termBaseLogPosition
                                                 : initialTermBaseLogPosition,
      logPosition, nowNs, timestamp, ctx.fileSyncLevel());
}

void Election::updateRecordingLog(std::int64_t nowNs) {
  ensureRecordingLogCoherent(m_leadershipTermId, m_logPosition, NULL_POSITION,
                             nowNs);
  m_logLeadershipTermId = m_leadershipTermId;
}

void Election::updateRecordingLogForReplication(
    std::int64_t leadershipTermId, std::int64_t termBaseLogPosition,
    std::int64_t logPosition, std::int64_t nowNs) {
  ensureRecordingLogCoherent(leadershipTermId, termBaseLogPosition, logPosition,
                             nowNs);
  m_logLeadershipTermId = leadershipTermId;
}

void Election::onCanvassPosition(std::int64_t logLeadershipTermId,
                                 std::int64_t logPosition,
  ...
  if (it != m_clusterMemberByIdMap.end() &&
      m_thisMember.id() != followerMemberId) {
    ...
    if (logLeadershipTermId < m_leadershipTermId) {
      if (service::Role::LEADER == m_consensusModuleAgent.roleValue()) {
        publishNewLeadershipTerm(
            *follower, logLeadershipTermId,
            m_consensusModuleAgent.calculateQuorumPosition(),
...
void Election::onRequestVote(std::int64_t logLeadershipTermId,
                             std::int64_t logPosition,
  ...
  if (candidateTermId <= m_candidateTermId) {
  ...
  } else if (compareLog(m_logLeadershipTermId, m_appendPosition,
                        logLeadershipTermId, logPosition) > 0) {
    m_candidateTermId = m_ctx.nodeStateFile()->proposeMaxCandidateTermId(
    ...
    auto it = m_clusterMemberByIdMap.find(candidateId);
    if (it != m_clusterMemberByIdMap.end() &&
        service::Role::LEADER == m_consensusModuleAgent.roleValue()) {
      ClusterMember *candidateMember = it->second;
      publishNewLeadershipTerm(*candidateMember, logLeadershipTermId,
                               m_consensusModuleAgent.calculateQuorumPosition(),
                               m_ctx.clusterClock()->time());
    }
...
void Election::onVote(std::int64_t candidateTermId,
  ...
  if (ElectionState::CANDIDATE_BALLOT == m_state &&
    ...
    if (it != m_clusterMemberByIdMap.end()) {
      ...
      follower->candidateTermId(candidateTermId)
          ...
          .vote(std::make_shared<bool>(vote));
    }
  }
}

void Election::onAppendPosition(std::int64_t leadershipTermId,
                                std::int64_t logPosition,
                                std::int32_t followerMemberId,
                                std::int16_t flags) {
  if (ElectionState::INIT == m_state) {
  ...
  if (leadershipTermId <= m_leadershipTermId) {
    ...
    if (it != m_clusterMemberByIdMap.end()) {
      ...
      m_consensusModuleAgent.trackCatchupCompletion(follower, leadershipTermId,
                                                    flags);
    }
  }
}

void Election::onCommitPosition(std::int64_t leadershipTermId,
                                std::int64_t logPosition,
  ...
  if (m_leaderMember && m_leaderMember->id() == leaderMemberId) {
    ...
    if (ElectionState::FOLLOWER_LOG_REPLICATION == state) {
      m_replicationDeadlineNs =
          m_ctx.clusterClock()->timeNanos() + m_ctx.leaderHeartbeatTimeoutNs();
    }
  } else if (leadershipTermId > m_leadershipTermId &&
             ElectionState::LEADER_READY == state) {
    throw ClusterEvent(
        std::string("new leader detected due to commit position - ") +
            " memberId=" + std::to_string(m_thisMember.id()) +
            " this.leadershipTermId=" + std::to_string(m_leadershipTermId) +
            " this.leaderMemberId=" +
            std::to_string(m_leaderMember ? m_leaderMember->id()
                                          : aeron::NULL_VALUE) +
...
void Election::onReplayNewLeadershipTermEvent(
    std::int64_t leadershipTermId, std::int64_t logPosition,
  ...
  if (ElectionState::FOLLOWER_CATCHUP == m_state ||
      ElectionState::FOLLOWER_REPLAY == m_state) {
    const std::int64_t nowNs = m_ctx.clusterClock()->convertToNanos(timestamp);
    ensureRecordingLogCoherent(leadershipTermId, termBaseLogPosition,
                               NULL_POSITION, nowNs);
...
void Election::onTruncateLogEntry(
    std::int32_t memberId, ElectionState state,
    std::int64_t logLeadershipTermId, std::int64_t leadershipTermId,
    std::int64_t candidateTermId, std::int64_t commitPosition,
    std::int64_t logPosition, std::int64_t appendPosition,
    std::int64_t oldPosition, std::int64_t newPosition) {
  // truncateLogEntry is a placeholder - log truncation would be implemented
  // here
...
std::int32_t Election::leaderLogReplication(std::int64_t nowNs) {
  std::int32_t workCount = 0;

  m_thisMember.logPosition(m_appendPosition).timeOfLastAppendPositionNs(nowNs);

  const std::int64_t quorumPosition =
      m_consensusModuleAgent.calculateQuorumPosition();
...
std::int32_t Election::leaderReplay(std::int64_t nowNs) {
  std::int32_t workCount = 0;

  if (!m_logReplay) {
    if (m_logPosition < m_appendPosition) {
      m_logReplay = m_consensusModuleAgent.createLogReplay(m_logPosition,
                                                           m_appendPosition);
    } else {
    ...
    m_thisMember.leadershipTermId(m_leadershipTermId)
        .logPosition(m_appendPosition);
  } else {
...
std::int32_t Election::leaderInit(std::int64_t nowNs) {
  m_consensusModuleAgent.joinLogAsLeader(m_leadershipTermId, m_logPosition,
                                         m_logSessionId, m_isLeaderStartup);
...
std::int32_t Election::leaderReady(std::int64_t nowNs) {
  std::int32_t workCount = m_consensusModuleAgent.updateLeaderPositionPublic(
      nowNs, m_appendPosition);
  workCount += publishNewLeadershipTermOnInterval(
      m_consensusModuleAgent.calculateQuorumPosition(), nowNs);
  ...
  if (hasVoters || (nowNs >= (m_timeOfLastStateChangeNs +
                              m_ctx.leaderHeartbeatTimeoutNs()) &&
                    hasQuorum)) {
    if (m_consensusModuleAgent.appendNewLeadershipTermEvent(nowNs)) {
      m_consensusModuleAgent.electionCompletePublic(nowNs);
...
std::int32_t Election::followerLogReplication(std::int64_t nowNs) {
  std::int32_t workCount = 0;
  ...
  if (!m_logReplication) {
    if (m_appendPosition < m_replicationStopPosition) {
      m_logReplication = m_consensusModuleAgent.newLogReplication(
          ...
          m_leaderMember->archiveResponseEndpoint(), m_leaderRecordingId,
          m_replicationStopPosition, nowNs);
      m_replicationDeadlineNs = nowNs + m_ctx.leaderHeartbeatTimeoutNs();
      workCount++;
    } else {
      updateRecordingLogForReplication(m_replicationLeadershipTermId,
                                       m_replicationTermBaseLogPosition,
                                       m_replicationStopPosition, nowNs);
      state(ElectionState::CANVASS, nowNs, "");
    }
  } else {
    workCount += m_consensusModuleAgent.pollArchiveEventsPublic();
    workCount += m_logReplication->poll(nowNs);
    const bool replicationDone = m_logReplication->hasReplicationEnded() &&
                                 m_logReplication->hasStopped();
...
std::int32_t Election::followerReplay(std::int64_t nowNs) {
  std::int32_t workCount = 0;
  ...
  if (!m_logReplay) {
    if (m_logPosition < m_appendPosition) {
      ...
      m_logReplay = m_consensusModuleAgent.createLogReplay(
          m_logPosition, std::min(m_appendPosition, m_notifiedCommitPosition));
      workCount++;
    } else {
      state((NULL_POSITION != m_catchupJoinPosition)
                ? ElectionState::FOLLOWER_CATCHUP_INIT
                : ElectionState::FOLLOWER_LOG_INIT,
            nowNs, "nothing to replay");
      workCount++;
    }
  } else {
    workCount += m_logReplay->doWork();
    if (m_logReplay->isDone()) {
      ...
      if (m_logPosition == m_appendPosition) {
        state((NULL_POSITION != m_catchupJoinPosition)
                  ? ElectionState::FOLLOWER_CATCHUP_INIT
                  : ElectionState::FOLLOWER_LOG_INIT,
              nowNs, "log replay done");
      } else {
        state(ElectionState::CANVASS, nowNs,
              "incomplete log replay: logPosition=" +
                  std::to_string(m_logPosition) +
                  " appendPosition=" + std::to_string(m_appendPosition));
      }
    }
  }
...
std::int32_t Election::followerCatchupInit(std::int64_t nowNs) {
  if (!m_logSubscription) {
  ...
  if (endpoint.length() >= 2 &&
      endpoint.substr(endpoint.length() - 2) == ":0") {
    ...
    if (!resolvedEndpoint.empty()) {
      ...
      if (i != std::string::npos) {
        catchupEndpoint = endpoint.substr(0, endpoint.length() - 2) +
                          resolvedEndpoint.substr(i);
      }
    }
  } else {
  ...
  if (!catchupEndpoint.empty() && sendCatchupPosition(catchupEndpoint)) {
    ...
    state(ElectionState::FOLLOWER_CATCHUP_AWAIT, nowNs, "");
             ...
             (m_timeOfLastStateChangeNs + m_ctx.leaderHeartbeatTimeoutNs())) {
...
std::int32_t Election::followerCatchupAwait(std::int64_t nowNs) {
  std::int32_t workCount = 0;
  ...
  if (image) {
    ...
    if (m_consensusModuleAgent.tryJoinLogAsFollower(image, m_isLeaderStartup,
                                                    nowNs)) {
      state(ElectionState::FOLLOWER_CATCHUP, nowNs, "");
      workCount++;
    } else if (ChannelEndpointStatus::CHANNEL_ENDPOINT_ERRORED ==
               m_logSubscription->channelStatus()) {
      const std::string message = "failed to add catchup log as follower - " +
                                  m_logSubscription->channel();
      throw ClusterException(message, SOURCEINFO);
    } else if (nowNs >=
               (m_timeOfLastStateChangeNs + m_ctx.leaderHeartbeatTimeoutNs())) {
      throw TimeoutException("failed to join catchup log as follower",
                             SOURCEINFO);
    }
  } else if (nowNs >=
             (m_timeOfLastStateChangeNs + m_ctx.leaderHeartbeatTimeoutNs())) {
...
std::int32_t Election::followerCatchup(std::int64_t nowNs) {
  std::int32_t workCount =
      m_consensusModuleAgent.catchupPoll(m_notifiedCommitPosition, nowNs);

  if (m_consensusModuleAgent.liveLogDestination().empty() &&
      m_consensusModuleAgent.isCatchupNearLive(
          std::max(m_catchupJoinPosition, m_notifiedCommitPosition))) {
  ...
  if (position >= m_catchupJoinPosition &&
      ...
      m_consensusModuleAgent.catchupLogDestination().empty() &&
      ConsensusModuleAgent::AgentState::SNAPSHOT !=
          m_consensusModuleAgent.agentState()) {
...
std::int32_t Election::followerLogInit(std::int64_t nowNs) {
  if (!m_logSubscription) {
...
std::int32_t Election::followerLogAwait(std::int64_t nowNs) {
  std::int32_t workCount = 0;
  ...
  if (image) {
    ...
    const std::string message =
        "failed to add live log as follower - " + m_logSubscription->channel();
    throw ClusterException(message, SOURCEINFO);
  } else if (nowNs >=
             (m_timeOfLastStateChangeNs + m_ctx.leaderHeartbeatTimeoutNs())) {
...
std::int32_t Election::followerReady(std::int64_t nowNs) {
  if (m_consensusPublisher.appendPosition(
          m_leaderMember->publication(), m_leadershipTermId, m_logPosition,
          m_thisMember.id(), APPEND_POSITION_FLAG_NONE)) {
    m_consensusModuleAgent.electionCompletePublic(nowNs);
    state(ElectionState::CLOSED, nowNs, "");
  } else if (nowNs >=
             (m_timeOfLastStateChangeNs + m_ctx.leaderHeartbeatTimeoutNs())) {
    throw TimeoutException("ready follower failed to notify leader",
                           SOURCEINFO);
  }
...
std::int32_t
Election::publishNewLeadershipTermOnInterval(std::int64_t quorumPosition,
                                             std::int64_t nowNs) {
  std::int32_t workCount = 0;

  if (hasUpdateIntervalExpired(nowNs, m_ctx.leaderHeartbeatIntervalNs())) {
    m_timeOfLastUpdateNs = nowNs;
    publishNewLeadershipTerm(quorumPosition, m_ctx.clusterClock()->time());
    workCount++;
  }
...
std::int32_t
Election::publishCommitPositionOnInterval(std::int64_t quorumPosition,
                                          std::int64_t nowNs) {
  std::int32_t workCount = 0;

  if (m_lastPublishedCommitPosition < quorumPosition ||
      (m_lastPublishedCommitPosition == quorumPosition &&
       hasIntervalExpired(nowNs, m_timeOfLastCommitPositionUpdateNs,
                          m_ctx.leaderHeartbeatIntervalNs()))) {
    m_timeOfLastCommitPositionUpdateNs = nowNs;
    m_lastPublishedCommitPosition = quorumPosition;
    m_consensusModuleAgent.publishCommitPosition(quorumPosition,
                                                 m_leadershipTermId);
    workCount++;
  }
...
void Election::publishNewLeadershipTerm(std::int64_t quorumPosition,
                                        std::int64_t timestamp) {
  for (auto &member : m_clusterMembers) {
    publishNewLeadershipTerm(member, m_logLeadershipTermId, quorumPosition,
                             timestamp);
  }
}

void Election::publishNewLeadershipTerm(ClusterMember &member,
                                        std::int64_t logLeadershipTermId,
                                        std::int64_t quorumPosition,
                                        std::int64_t timestamp) {
  if (member.id() != m_thisMember.id() && 0 != m_logSessionId) {
    auto recordingLog = m_ctx.recordingLog();
    auto logNextTermEntry =
        recordingLog->findTermEntry(logLeadershipTermId + 1);

    const std::int64_t nextLeadershipTermId =
        logNextTermEntry ? logNextTermEntry->leadershipTermId
                         : m_leadershipTermId;
    const std::int64_t nextTermBaseLogPosition =
        logNextTermEntry ? logNextTermEntry->termBaseLogPosition
                         : m_appendPosition;

    const std::int64_t nextLogPosition =
        logNextTermEntry ? ((NULL_POSITION != logNextTermEntry->logPosition)
                                ? logNextTermEntry->logPosition
                                : m_appendPosition)
                         : NULL_POSITION;

    m_consensusPublisher.newLeadershipTerm(
        member.publication(), logLeadershipTermId, nextLeadershipTermId,
        nextTermBaseLogPosition, nextLogPosition, m_leadershipTermId,
        m_appendPosition, m_appendPosition, quorumPosition,
        m_consensusModuleAgent.logRecordingId(), timestamp, m_thisMember.id(),
        m_logSessionId, m_ctx.appVersion(), m_isLeaderStartup);
  }
}

std::int32_t Election::publishFollowerReplicationPosition(std::int64_t nowNs) {
  const std::int64_t position = m_logReplication->position();
  if (position > m_appendPosition ||
      (position == m_appendPosition &&
       hasUpdateIntervalExpired(nowNs, m_ctx.leaderHeartbeatIntervalNs()))) {
    if (m_consensusPublisher.appendPosition(
            m_leaderMember->publication(), m_leadershipTermId, position,
            m_thisMember.id(), APPEND_POSITION_FLAG_NONE)) {
      m_appendPosition = position;
      m_timeOfLastUpdateNs = nowNs;
      return 1;
...
std::int32_t Election::publishFollowerAppendPosition(std::int64_t nowNs) {
  if (m_lastPublishedAppendPosition != m_appendPosition ||
      hasUpdateIntervalExpired(nowNs, m_ctx.leaderHeartbeatIntervalNs())) {
    if (m_consensusPublisher.appendPosition(
            m_leaderMember->publication(), m_leadershipTermId, m_appendPosition,
            m_thisMember.id(), APPEND_POSITION_FLAG_NONE)) {
...
bool Election::sendCatchupPosition(const std::string &catchupEndpoint) {
  return m_consensusPublisher.catchupPosition(
      m_leaderMember->publication(), m_leadershipTermId, m_logPosition,
      m_thisMember.id(), catchupEndpoint);
}

void Election::addCatchupLogDestination() {
  // Create destination URI by appending endpoint to channel
  auto channelUri = ChannelUri::parse(m_ctx.logPublicationChannel());
  channelUri->put(ENDPOINT_PARAM_NAME, m_thisMember.catchupEndpoint());
  const std::string destination = channelUri->toString();
  m_logSubscription->addDestination(destination);
  m_consensusModuleAgent.catchupLogDestination(destination);
}

void Election::addLiveLogDestination() {
  std::string destination;
  if (m_ctx.isLogMdc()) {
    auto channelUri = ChannelUri::parse(m_ctx.logPublicationChannel());
    channelUri->put(ENDPOINT_PARAM_NAME, m_thisMember.logEndpoint());
    destination = channelUri->toString();
  } else {
    destination = m_ctx.logPublicationChannel();
  }
  m_logSubscription->addDestination(destination);
  m_consensusModuleAgent.liveLogDestination(destination);
}

std::shared_ptr<Subscription> Election::addFollowerSubscription() {
  auto aeron = m_ctx.aeron();
  auto logChannelUri = ChannelUri::parse(m_ctx.logPublicationChannel());

  const std::int64_t tag1 = aeron->nextCorrelationId();
  const std::int64_t tag2 = aeron->nextCorrelationId();

  // Get socket buffer sizes from the parsed channel URI if present
  auto socketRcvbufStr = logChannelUri->get(SOCKET_RCVBUF_PARAM_NAME);
  auto rcvWindowStr = logChannelUri->get(RECEIVER_WINDOW_LENGTH_PARAM_NAME);
  ...
  // Apply socket buffer lengths if present in source channel
  if (!socketRcvbufStr.empty()) {
    builder.socketRcvbufLength(
        static_cast<std::uint32_t>(std::stoul(socketRcvbufStr)));
  }
  if (!rcvWindowStr.empty()) {
    builder.receiverWindowLength(
        static_cast<std::uint32_t>(std::stoul(rcvWindowStr)));
  }

  const std::string channel = builder.build();

  // addSubscription returns a registration ID, need to use async or find
  // subscription
  std::int64_t registrationId =
      aeron->addSubscription(channel, m_ctx.logStreamId());
  // Wait for subscription to be available
  while (true) {
    auto subscription = aeron->findSubscription(registrationId);
    if (subscription) {
      return subscription;
    }
...
void Election::onNewLeadershipTerm(
    ...
    std::int32_t leaderMemberId, std::int32_t logSessionId, bool isStartup) {
  if (ElectionState::INIT == m_state) {
    return;
  }
  ...
  if (it == m_clusterMemberByIdMap.end() ||
      (leaderMemberId == m_thisMember.id() &&
  ...
  if (((ElectionState::FOLLOWER_BALLOT == m_state ||
        ElectionState::CANDIDATE_BALLOT == m_state) &&
    ...
    if (logLeadershipTermId == m_logLeadershipTermId) {
      if (NULL_POSITION != nextTermBaseLogPosition &&
          nextTermBaseLogPosition < m_appendPosition) {
        onTruncateLogEntry(m_thisMember.id(), m_state, logLeadershipTermId,
                           m_leadershipTermId, m_candidateTermId,
                           m_ctx.commitPositionCounter()->get(), m_logPosition,
                           m_appendPosition, m_appendPosition,
      ...
      m_catchupJoinPosition =
          (m_appendPosition < logPosition) ? logPosition : NULL_POSITION;
      ...
      if (m_appendPosition < termBaseLogPosition) {
        if (aeron::NULL_VALUE != nextLeadershipTermId) {
          if (m_appendPosition < nextTermBaseLogPosition) {
            ...
            state(ElectionState::FOLLOWER_LOG_REPLICATION,
                  m_ctx.clusterClock()->timeNanos(), "");
          ...
          throw ClusterException(
              "invalid newLeadershipTerm - this.appendPosition=" +
                  std::to_string(m_appendPosition) + " < termBaseLogPosition=" +
                  std::to_string(termBaseLogPosition) +
                  " and nextLeadershipTermId=" +
                  std::to_string(nextLeadershipTermId) +
                  ", logLeadershipTermId=" +
                  std::to_string(logLeadershipTermId) +
                  ", nextTermBaseLogPosition=" +
                  std::to_string(nextTermBaseLogPosition) +
...
void Election::onRecordingSignal(std::int64_t correlationId,
...
void Election::state(ElectionState newState, std::int64_t nowNs,
  ...
  if (newState != m_state) {
    ...
    case ElectionState::CANVASS:
      resetMembers();
      m_consensusModuleAgent.setRole(service::Role::FOLLOWER);
      break;

    case ElectionState::CANDIDATE_BALLOT:
      m_consensusModuleAgent.setRole(service::Role::CANDIDATE);
      break;

    case ElectionState::LEADER_LOG_REPLICATION:
      m_consensusModuleAgent.setRole(service::Role::LEADER);
      m_logSessionId =
