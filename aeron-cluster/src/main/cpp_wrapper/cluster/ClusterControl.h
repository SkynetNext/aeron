...
public:
     ...
     */
    static constexpr std::int32_t CONTROL_TOGGLE_TYPE_ID = AeronCounters::CLUSTER_CONTROL_TOGGLE_TYPE_ID;
    ...
    {
        if (code(targetState) == code(ToggleState::RESUME) && controlToggle->get() == code(ToggleState::SUSPEND))
        {
            return controlToggle->compareAndSet(code(ToggleState::SUSPEND), code(ToggleState::RESUME));
        }
