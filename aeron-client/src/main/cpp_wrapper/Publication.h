...
/**
 * Supplies the reserved value field for a data frame header. The returned value
 * will be set in the header as Little Endian format.
 *
 * This will be called as the last action of encoding a data frame right before
 * the length is set. All other fields in the header plus the body of the frame
 * will have been written at the point of supply.
 *
 ...
 */
/**
 * Aeron Publisher API for sending messages to subscribers of a given channel
 * and streamId pair. Publishers are created via an {@link Aeron} object, and
 * messages are sent via an offer method or a claim and commit method
 * combination. <p> The APIs for tryClaim and offer are non-blocking and
 * threadsafe. <p> Note: Publication instances are threadsafe and can be shared
 * between publisher threads.
...
public:
  ...
  Publication(aeron_t *aeron, aeron_publication_t *publication)
      : m_aeron(aeron), m_publication(publication) {
  ...
  ~Publication() { aeron_publication_close(m_publication, nullptr, nullptr); }
   ...
   */
  inline const std::string &channel() const { return m_channel; }
   ...
   */
  inline std::int32_t streamId() const { return m_constants.stream_id; }

  /**
   * Session under which messages are published. Identifies this Publication
   * instance.
   *
   * @return the session id for this publication.
   */
  inline std::int32_t sessionId() const { return m_constants.session_id; }

  /**
   * The initial term id assigned when this Publication was created. This can be
   * used to determine how many terms have passed since creation.
   *
  ...
  /**
   * Get the original registration used to register this Publication with the
   * media driver by the first publisher.
   *
  ...
  /**
   * Registration Id returned by Aeron::addPublication when this Publication was
   * added.
   *
  ...
  /**
   * Is this Publication the original instance added to the driver? If not then
   * it was added after another client has already added the publication.
   *
  ...
  /**
   * Number of bits to right shift a position to get a term count for how far
   * the stream has progressed.
   *
   * @return of bits to right shift a position to get a term count for how far
   * the stream has progressed.
   */
  ...
  /**
   * Get the current position to which the publication has advanced for this
   * stream.
   *
   * @return the current position to which the publication has advanced for this
   * stream or {@link CLOSED}.
   */
  ...
  /**
   * Get the position limit beyond which this {@link Publication} will be back
   * pressured.
   *
   * This should only be used as a guide to determine when back pressure is
   * likely to be applied.
   *
   * @return the position limit beyond which this {@link Publication} will be
   * back pressured.
   */
  ...
  /**
   * Available window for offering into a publication before the {@link
   * #positionLimit()} is reached.
   *
   * @return  window for offering into a publication before the {@link
   * #positionLimit()} is reached. If the publication is closed then {@link
   * #CLOSED} will be returned.
   */
  inline std::int64_t availableWindow() const {
    const std::int64_t limit = publicationLimit();
    return AERON_PUBLICATION_CLOSED != limit ? limit - position()
  ...
  /**
   * Fetches the local socket addresses for this publication. If the channel is
   * not
   * {@link
   * aeron::concurrent::status::ChannelEndpointStatus::CHANNEL_ENDPOINT_ACTIVE},
   ...
   * @return The new stream position, otherwise {@link #NOT_CONNECTED}, {@link
   * #BACK_PRESSURED},
   * {@link #ADMIN_ACTION} or {@link #CLOSED}.
   */
  inline std::int64_t
  offer(const concurrent::AtomicBuffer &buffer, util::index_t offset,
        util::index_t length,
        const on_reserved_value_supplier_t &reservedValueSupplier) {
    std::int64_t position = aeron_publication_offer(
        m_publication, buffer.buffer() + offset,
        static_cast<std::size_t>(length), reservedValueSupplierCallback,
  ...
  inline std::int64_t offer(const concurrent::AtomicBuffer &buffer,
                            util::index_t offset, util::index_t length) {
   ...
   *
   * @param buffer containing message.
   * @return The new stream position on success, otherwise {@link
   * BACK_PRESSURED} or {@link NOT_CONNECTED}.
   */
  inline std::int64_t offer(const concurrent::AtomicBuffer &buffer) {
  ...
  std::int64_t offer(BufferIterator startBuffer, BufferIterator lastBuffer,
                     const on_reserved_value_supplier_t &reservedValueSupplier =
                         DEFAULT_RESERVED_VALUE_SUPPLIER) {
    ...
    const std::int64_t position = aeron_publication_offerv(
        m_publication, iov.data(), iov.size(), reservedValueSupplierCallback,
        (void *)&reservedValueSupplier);
   ...
   * {@link #ADMIN_ACTION} or {@link #CLOSED}.
   */
  std::int64_t offer(const concurrent::AtomicBuffer *buffers,
                     std::size_t length,
                     const on_reserved_value_supplier_t &reservedValueSupplier =
                         DEFAULT_RESERVED_VALUE_SUPPLIER) {
  ...
  std::int64_t offer(const std::array<concurrent::AtomicBuffer, N> &buffers,
                     const on_reserved_value_supplier_t &reservedValueSupplier =
                         DEFAULT_RESERVED_VALUE_SUPPLIER) {
  ...
  /**
   * Non-blocking publish of a simple const data buffer and length containing a
   * message
   *
   ...
   * {@link #ADMIN_ACTION} or {@link #CLOSED}.
   */
  std::int64_t offer(const std::uint8_t *buffer, std::size_t length,
                     const on_reserved_value_supplier_t &reservedValueSupplier =
                         DEFAULT_RESERVED_VALUE_SUPPLIER) {
    std::int64_t position = aeron_publication_offer(
        m_publication, buffer, static_cast<std::size_t>(length),
        reservedValueSupplierCallback, (void *)&reservedValueSupplier);
  ...
  /**
   * Try to claim a range in the publication log into which a message can be
   * written with zero copy semantics. Once the message has been written then
   * {@link BufferClaim#commit()} should be called thus making it available. <p>
   * <b>Note:</b> This method can only be used for message lengths less than MTU
   * length minus header.
   *
   * @code
   *     BufferClaim bufferClaim; // Can be stored and reused to avoid
   * allocation
   *
   ...
   * #BACK_PRESSURED},
   * {@link #ADMIN_ACTION} or {@link #CLOSED}.
   * @throws IllegalArgumentException if the length is greater than max payload
   * length within an MTU.
   * @see BufferClaim::commit
   */
  inline std::int64_t
  tryClaim(util::index_t length,
           concurrent::logbuffer::BufferClaim &bufferClaim) {
    ...
    // This is slight cheat, we are assuming that the frame_header and data
    // pointers index into the same contiguous block of memory.
  ...
  AsyncDestination *addDestinationAsync(const std::string &endpointChannel) {
    ...
    if (aeron_publication_async_add_destination(&async, m_aeron, m_publication,
                                                endpointChannel.c_str()) < 0) {
      AERON_MAP_ERRNO_TO_SOURCED_EXCEPTION_AND_THROW;
    }
  ...
  std::int64_t addDestination(const std::string &endpointChannel) {
    ...
    std::int64_t correlationId =
        aeron_async_destination_get_registration_id(async);
  ...
  /**
   * Remove a previously added destination manually from a
   * multi-destination-cast Publication.
   *
  ...
  AsyncDestination *removeDestinationAsync(const std::string &endpointChannel) {
    AsyncDestination *async = nullptr;
    if (aeron_publication_async_remove_destination(
   ...
   */
  AsyncDestination *
  removeDestinationAsync(std::int64_t destinationRegistrationId) {
    AsyncDestination *async = nullptr;
  ...
  /**
   * Remove a previously added destination manually from a
   * multi-destination-cast Publication by destinationRegistrationId.
   *
   * @param destinationRegistrationId for the destination to remove
   * @return correlation id for the remove command
   */
  ...
  /**
   * Retrieve the status of the associated add or remove destination operation
   * with the given correlationId.
   *
   * This method is non-blocking.
   *
   * The value returned is dependent on what has occurred with respect to the
   * media driver:
   *
   * - If the correlationId is unknown, then an exception is thrown.
   * - If the media driver has not answered the add/remove command, then a false
   * is returned.
   * - If the media driver has successfully added or removed the destination
   * then true is returned.
   * - If the media driver has returned an error, this method will throw the
   * error returned.
   *
   ...
   * @see Publication::removeDestination
   *
   * @param correlationId of the add/remove command returned by
   * Publication::addDestination or Publication::removeDestination
   * @return true for added or false if not.
   */
  ...
  /// @cond HIDDEN_SYMBOLS
  aeron_publication_t *publication() { return m_publication; }
  /// @endcond
...
private:
  ...
  static std::int64_t reservedValueSupplierCallback(void *clientd,
                                                    std::uint8_t *buffer,
                                                    std::size_t frame_length) {
    on_reserved_value_supplier_t &supplier =
        *static_cast<on_reserved_value_supplier_t *>(clientd);
