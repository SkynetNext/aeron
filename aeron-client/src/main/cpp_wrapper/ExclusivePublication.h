 ...
 * and streamId pair. ExclusivePublications each get their own session id so
 * multiple can be concurrently active on the same media driver as independent
 * streams.
 *
 * {@link ExclusivePublication}s are created via the {@link
 * Aeron#addExclusivePublication(String, int)} method, and messages are sent via
 * one of the {@link #offer(DirectBuffer)} methods, or a
 * {@link #tryClaim(int, ExclusiveBufferClaim)} and {@link
 * ExclusiveBufferClaim#commit()} method combination.
 *
 * {@link ExclusivePublication}s have the potential to provide greater
 * throughput than {@link Publication}s.
 *
 * The APIs tryClaim and offer are non-blocking.
 *
 * <b>Note:</b> ExclusivePublication instances are NOT threadsafe for offer and
 * try claim methods but are for others.
 *
...
public:
  /// @cond HIDDEN_SYMBOLS
  ExclusivePublication(aeron_t *aeron,
                       aeron_exclusive_publication_t *publication)
      : m_aeron(aeron), m_publication(publication) {
    if (aeron_exclusive_publication_constants(m_publication, &m_constants) <
        0) {
  ...
  inline void close() {
    if (aeron_exclusive_publication_close(m_publication, nullptr, nullptr) <
        0) {
      AERON_MAP_ERRNO_TO_SOURCED_EXCEPTION_AND_THROW;
    }
  ...
  inline void revoke() {
    if (aeron_exclusive_publication_revoke(m_publication, nullptr, nullptr) <
        0) {
      AERON_MAP_ERRNO_TO_SOURCED_EXCEPTION_AND_THROW;
    }
   ...
   * @return true.
   */
  static constexpr bool isOriginal() { return true; }
  ...
  inline std::int64_t publicationLimit() const {
    std::int64_t limit =
        aeron_exclusive_publication_position_limit(m_publication);
  ...
  std::vector<std::string> localSocketAddresses() const {
    ...
    if (aeron_exclusive_publication_local_sockaddrs(m_publication, &iov, 1) <
        0) {
      AERON_MAP_ERRNO_TO_SOURCED_EXCEPTION_AND_THROW;
    }
  ...
  inline std::int64_t offer(const concurrent::AtomicBuffer &buffer,
    ...
    std::int64_t position = aeron_exclusive_publication_offer(
        m_publication, buffer.buffer() + offset,
        static_cast<std::size_t>(length), nullptr, nullptr);
  ...
  AsyncDestination *addDestinationAsync(const std::string &endpointChannel) {
    ...
    if (aeron_exclusive_publication_async_add_destination(
  ...
  /// @cond HIDDEN_SYMBOLS
  aeron_exclusive_publication_t *publication() { return m_publication; }
  /// @endcond
