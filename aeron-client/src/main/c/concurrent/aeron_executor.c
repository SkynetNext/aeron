...
aeron_executor_task_acquire(aeron_executor_t *executor,
                            aeron_executor_task_on_execute_func_t on_execute,
                            aeron_executor_task_on_complete_func_t on_complete,
                            void *clientd, bool shutdown) {
...
static void *aeron_executor_dispatch(void *arg) {
  ...
  do {
    task = (aeron_executor_task_t *)aeron_blocking_linked_queue_take_ex(
        &executor->queue, &node);
    ...
    task->result = (NULL == task->on_execute)
                       ? 0
                       : task->on_execute(task->clientd, executor->clientd);
    ...
    if (USE_RETURN_QUEUE(executor)) {
      ...
      // When not using return queue, shutdown tasks should be released
      // immediately
...
int aeron_executor_init(
    aeron_executor_t *executor, bool async,
  ...
  if (async) {
    ...
    if ((result = aeron_thread_create(&executor->dispatch_thread, &attr,
                                      aeron_executor_dispatch, executor)) !=
        0) {
...
int aeron_executor_close(aeron_executor_t *executor) {
  ...
  // at this point, the executor thread has already been joined, so the shutdown
  // task must already be on the return queue
  aeron_linked_queue_node_t *node;
  bool shutdown = false;
...
int aeron_executor_submit(aeron_executor_t *executor,
                          aeron_executor_task_on_execute_func_t on_execute,
                          aeron_executor_task_on_complete_func_t on_complete,
                          void *clientd) {
  ...
  if (executor->async) {
    ...
    task = aeron_executor_task_acquire(executor, on_execute, on_complete,
                                       clientd, false);
    if (NULL == task) {
      AERON_APPEND_ERR("%s", "");
      return -1;
  ...
  on_complete(result, aeron_errcode(), aeron_errmsg(), clientd,
              executor->clientd);
...
void aeron_executor_task_do_complete(aeron_executor_task_t *task) {
  ...
  if (NULL != task->on_complete) {
    ...
    if (NULL != task->executor) {
      task->on_complete(task->result, task->errcode, task->errmsg,
                        task->clientd, task->executor->clientd);
    }
  }
